...
1. **Verify the analyze gate before external issue sync**: Confirm `$speckit-analyze` ran after the current `tasks.md` and met the feature threshold (default: no unresolved critical/high findings and no blocking clarification requests). If current evidence is absent or stale, STOP and run `$speckit-analyze`; do not read or create GitHub issues yet.
