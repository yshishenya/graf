...
2. **Start independent review and check checklist status** (if FEATURE_DIR/checklists/ exists):
   - Before evaluating the gate, the orchestrator MUST automatically start a separate independent reviewer agent without waiting for separate user confirmation. The reviewer reads every checklist plus the active spec, plan, tasks, constitution and required guidance; it marks `[x]` only evidence-backed items, leaves unsupported items unchecked with reasons in a separate report, rereads the checklist and reports checked/unchecked totals.
   - The main implementation agent treats checklist markers as a read-only gate: scan checkbox state, report status, and ask before proceeding when needed; do NOT modify checklist files or markers. The reviewer may edit only reviewer-owned checklist files and its separate review report; it must not edit code, spec, plan, tasks, constitution, PRs, GitHub state, commits, releases or deployments.
...
Report implementation and tracker status separately. Local tests or checked tasks
are not merged/released acceptance. Read the repository tracker policy and list
which issues are fully closable, which require merge/release/manual evidence, and
why. For fully satisfied acceptance use explicit closing keywords in the PR;
`Refs` is only a relationship and never closes an issue. When merge and required
acceptance are already verified, run `$speckit-taskstoissues closeout` in projects
that provide the closeout validator. Otherwise report `implementation ready;
tracker pending` with the remaining gates. Do not report overall completion merely
because every local task checkbox is checked.
