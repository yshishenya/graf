...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `$speckit-plan`. An explicit skip is allowed only when the selected project risk lane makes clarification optional. Capture, privacy, auth, backend, infrastructure, deletion, diagnostics, and high-risk UX lanes MUST NOT skip clarify; STOP until the required clarification is complete.
...
5. Sequential questioning loop (interactive):
    ...
    - After the user answers:
       ...
       - Once satisfactory, immediately integrate the accepted answer and atomically save the spec as required by step 6; only then move to the next queued question.
