...
## [2026.09.02.3] - 2026-09-04

### Добавлено
- _Пока нет записей._

### Изменено
- Выпуск новых версий стал надёжнее: перед публикацией система проводит единую полную проверку (Фича 236, issue #6415).
- Ответы помощника стали понятнее и написаны простым языком (Фича 237, issue #6432).
...
### Изменено
- Выпуск новых версий теперь проходит единую автоматическую проверку в GitHub, чтобы перед публикацией подтвердить целостность всей версии (Фича 236, issue #6415).
- Ответы помощника стали понятнее: он отвечает на языке пользователя и избегает лишних сложных слов (Фича 237, issue #6432).
- Повысили надёжность обновления и запуска Dev-версии приложения: она открывается с правильным именем и значком, а откат возвращает рабочую версию (Фича 229, issue #6276).
...
- Closeout harness Feature 235 теперь сохраняет связь wrapped task rows и standalone issue mappings, принимает только canonical `Issue #N`, а незавершённые `[ ]` задачи не допускает к закрытию (Фича 235, issue #6385, tasks [T001, T002, T003, T004, T005, T006]); совместимость: Формат существующих `tasks.md` сохраняется; product runtime и production data не меняются; release notes: Проверка task↔issue работает для многострочных и canonical mapping rows и fail-closed отделяет umbrella issue; ограничения: Umbrella issue закрывается отдельно после общего release/operator closeout.
