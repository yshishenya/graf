...
- Серверный маршрут прослушивания поддерживает byte-range ответы, чтобы
  браузер мог быстро начинать воспроизведение и прыгать по таймкодам без
  прямых ссылок на хранилище (`feature:048`).

### Изменено
- Экран результата готовится к более удобному просмотру по образцу
  transcript-first интерфейсов: расшифровка остается главным содержимым,
  проигрыватель закреплен внизу, таймкоды ведут к нужному месту записи, а
  дорожки говорящих помогают быстро понять, где кто говорит (`feature:048`).
...
### Безопасность
- Прослушивание остается server-mediated: ответы не содержат signed URL,
  storage object key, приватные пути, сырой текст встречи или диагностический
  звук (`feature:048`).
