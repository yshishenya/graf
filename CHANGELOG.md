...
### Изменено
- Провалившийся выпуск больше не блокирует следующий: подготовка сводит фрагменты пережившей его доработки в один файл и оставляет в журнале ровно одну запись, а брошенный кандидат получает отдельную отметку об отмене, поэтому повторный запуск проходит без ручной расчистки. (Фича 271, issue #7127, tasks [T037, T038, T039]); совместимость: Изменений совместимости нет.; release notes: Внутреннее улучшение процесса выпуска, для пользователя ничего не меняется.; ограничения: Проверка на испорченный архив сохраняется: одинаковое имя файла при разных номерах доработки по-прежнему останавливает подготовку до изменений.
