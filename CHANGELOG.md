...
### Исправлено
- Lifecycle fence теперь учитывает неполные замены: такие результаты не
  публикуются и не затмевают последнюю подтверждённую расшифровку. Desktop и
  cabinet согласованно показывают processing/failed состояния (Фича 216,
  issue #6090).
...
- Dev-выкатка теперь всегда запускает GRAF Dev с Dev-иконкой вместо оставшегося процесса GRAF Local (Фича 229, issue #6276, tasks [T039]); совместимость: Формат Dev manifest не изменён; прямой installer теперь требует закрытого GRAF Dev; release notes: Promotion и rollback штатно перезапускают установленное Dev-приложение, обновляют его регистрацию в macOS и проверяют имя, канал и отдельную Dev-иконку до принятия кандидата.
