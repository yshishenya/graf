...
- В `052` найден и исправлен production-блокер обработки: `rec-api` теперь
  получает настройки dispatch в Temporal и доступ к MediaScribe secret, чтобы
  после upload finalize запускать обработку, а не оставлять записи в
  `not_submitted`.
...
### Документы
- Начата фича `052`: финальная доказательная проверка MVP-пути после `051`.
  Срез должен подтвердить свежий путь от установленного приложения до
  production-кабинета, сохраненные итоги, нижнюю шкалу спикеров, веб/desktop
  интерфейс и timing на длинной записи, либо честно оставить статус
  `pilot_blocked`.
- Зафиксирован текущий итог `052`: локальные web/mobile/embedded проверки
  подтверждают playback, seek, нижнюю шкалу спикеров и outcome rows; macOS
  приложение честно показывает состояние `Нужен вход`. Production owner review,
  stored outcomes и timing остаются открытыми P1-gates, поэтому MVP все еще
  `pilot_blocked`.
