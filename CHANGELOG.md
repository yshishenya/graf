...
### Изменено
- PR-проверка перенесена на GitHub Actions с pinned Spec Kit bootstrap; локальный CI сохранён как fallback (Фича 233, issue #6363, tasks [T001, T002, T003, T004, T005, T006, T007, T008]); совместимость: infra/scripts/ci-local.sh не удалён и может использоваться локально для диагностики; release notes: GitHub governance-fast становится обязательной автоматической проверкой PR; локальный запуск больше не является источником merge evidence.; ограничения: Полный CI остаётся отдельным gate для замороженного release candidate
- Укреплены bounded-выдача Feature ID, правила issue-closeout и единый GitHub PR gate (Фича 234, issue #6373, tasks [T001, T002, T003, T004, T005, T006]); совместимость: Локальный ci-local.sh сохранён как ручной fallback; production и прикладная логика не изменены; release notes: Онлайн allocator больше не сканирует всю историю issues, а закрытие issue с задачей требует проверяемого русского closure evidence; ограничения: Закрытие GitHub issue остаётся явным действием владельца после metadata-only проверки
- Closeout harness сохраняет task↔issue трассируемость для wrapped и standalone mapping rows и fail-closed проверяет завершённость задач. (Фича 235, issue #6385, tasks [T001, T002, T003, T004, T005, T006]); совместимость: Формат существующих tasks.md сохраняется; product runtime и production data не меняются.; release notes: Проверка task↔issue работает для многострочных и canonical mapping rows, принимает только Issue #N и отклоняет незавершённые задачи.; ограничения: Umbrella issue закрывается отдельно после общего release/operator closeout.
- Полная предрелизная проверка выполняется в GitHub Actions на замороженном кандидате; macOS-тесты изолированы, а подготовка и закрытие релиза не могут пропустить неопубликованные изменения или открытые issues. (Фича 236, issue #6415, tasks [T001, T002, T003, T004, T005, T006, T007, T008, T009, T010, T011, T012, T013, T014, T015, T016, T017, T018, T019]); совместимость: Поведение приложения не изменяется; локальная полная проверка остаётся для диагностики.; release notes: Добавлена одна проверяемая цепочка от последнего опубликованного релиза до полной GitHub-проверки, решения, публикации и фактического закрытия issues.; ограничения: Developer ID signing, Apple notarization и production smoke не выполняются на GitHub runner.
- Ответы агента должны быть написаны простым и понятным языком; в русскоязычных ответах — без необязательных англицизмов. (Фича 237, issue #6432, tasks [T001, T002, T003, T004]); совместимость: Работа приложения и существующие правила не меняются.; release notes: В корневые инструкции добавлено общее правило: отвечать на языке пользователя понятным языком, а в русскоязычных ответах избегать лишних англицизмов и кратко пояснять необходимые непонятные термины.; ограничения: Автоматическая проверка слов не добавлена; правило применяется агентом при формировании ответа.
...
- Повторная обработка получила единый спокойный экран ожидания и согласованное обновление расшифровки с плеером (Фича 213, issue #6365, tasks [T031, T032, T033, T034, T035, T036, T037]); совместимость: Миграция данных не требуется; прежний опубликованный результат сохраняется до успешной замены; release notes: Перед запуском GRAF предупреждает о сбросе ручных имён. Во время подготовки скрывает старую версию без промежуточных ошибок, при сбое возвращает её, а при успехе одновременно обновляет расшифровку и плеер.; ограничения: Имена спикеров, заданные вручную, намеренно не переносятся в новую версию; Полный CI остаётся отдельным gate замороженного release candidate
- Dev-выкатка запускает GRAF Dev с Dev-иконкой и восстанавливает только точные Docker images выбранного кандидата (Фича 229, issue #6276, tasks [T039, T041]); совместимость: Новые manifest содержат image ID для one-shot storage init; старые manifest читаются, но live rollback блокируется, если в них нет полного набора image IDs; release notes: Promotion и rollback штатно перезапускают установленное Dev-приложение, проверяют имя, канал и отдельную Dev-иконку, а Compose запускают только из immutable image IDs выбранного manifest.; ограничения: ["Изменение host-side orchestration или macOS mutation helpers требует отдельного operator cutover без удаления старого state; обычный server source связан с immutable image и компенсируется автоматически"]
- Локальные записи читаемо отображаются, открываются с Mac и однозначно заменяются серверной встречей после отправки; допустимые громкие пики больше не останавливают AEC3. (Фича 231, issue #6337, tasks [T001, T002, T003, T004, T005, T006, T007, T008, T009, T010, T011, T012, T013, T014]); совместимость: Новый API и миграция БД не требуются; старые локальные queue items уточняются при очередном scan.; release notes: Исправлены UTF-8, пиктограмма, открытие, фактическая длительность и local-to-server handoff; точные причины отказа AEC сохраняются, а четыре подтверждённо неценные старые записи удалены локально.; ограничения: Неполный очищенный префикс остаётся только локальным и не отправляется автоматически.
- GRAF снова запускается после обновления: исправлен путь к встроенному ресурсу определения встреч (Фича 232, issue #6338, tasks [T001, T002, T003, T004, T005, T006, T007, T008, T009, T010, T011, T012, T013, T014, T015, T016, T017, T018, T019, T020, T021, T022]); совместимость: Пользователям v2026.09.02.1 нужна ручная установка нового PKG или ZIP; предыдущие рабочие версии обновляются через Sparkle; release notes: Исправлен аварийный выход GRAF при запуске после обновления. Релизная проверка теперь запускает точный бинарник готового GRAF.app до создания appcast. Настройки автозаписи, Record/Stop, данные и серверная часть не менялись.; ограничения: v2026.09.02.1 завершается до надёжного запуска встроенного updater и не может сама гарантированно восстановиться
