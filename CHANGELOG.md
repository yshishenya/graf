...
### Добавлено
- Feature `087-own-media-upload-processing`: добавлен API-first путь
...
- Feature `090-manual-media-upload-ui`: в web cabinet и embedded desktop
  meetings добавлена ручная загрузка одного медиафайла с выбором файла,
  длительностью, прогрессом передачи, отменой до принятия сервером и переходом
  в обычную встречу.

### Изменено
- Processing теперь различает `dual_track` desktop записи и `single_track`
  manual media uploads: однодорожечные загрузки отправляются в серверный
  MediaScribe endpoint `POST /v1/audio/transcriptions`, а desktop записи
  продолжают использовать dual-track контракт.
- Manual uploads теперь отображаются в списке/detail как обычные встречи с
  source `manual_upload`, русской меткой происхождения `медиа` и теми же
  статусами отправки, обработки и готовности, что и остальные поверхности
  просмотра встреч.
...
### Безопасность
- Feature `090-manual-media-upload-ui`: для cookie-authenticated cabinet upload
  добавлен отдельный CSRF-protected route `POST /api/v1/cabinet/media-uploads`;
  старый embedded-контекст только с заголовками получает безопасное состояние
  входа или недоступности и не используется как граница небезопасной загрузки.

### Документы
- _Пока нет записей._

### Операции
- Для выпуска требуется проверка focused server/macOS сценариев, скан на
  запрещенное содержимое и полный локальный gate `infra/scripts/ci-local.sh`.
