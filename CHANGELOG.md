...
### Изменено
- Уточнен язык публичного лендинга и release notes: основной путь называется
  входом на страницу входа, без отдельной публичной регистрации.
...
### Документы
- B2C brief лендинга теперь описывает self-serve путь через страницу входа, а
  не через отдельный процесс авторизации или регистрации.
...
### Изменено
- Публичная главная страница стала короче и точнее: CTA `Начать` ведет на
  страницу входа, шумный proof-row и подпись под кнопкой убраны, а блок
  совместимости заменен на движущуюся wordmark-полосу с примерами российских и
  глобальных сервисов для звонков без центрального бейджа.
...
### Документы
- Обновлен B2C brief лендинга: зафиксирован путь `лендинг -> вход -> кабинет
  -> скачать приложение -> включить автозапись`, без публичного обещания
  скачивания до появления реального installer route или release asset.
...
- Публичная главная страница `rec.2brain.pro`: B2C-лендинг GRAF с прямым
  переходом на страницу входа, локальными product visuals и блоком готовых
  итогов встречи без demo/pilot flow.
