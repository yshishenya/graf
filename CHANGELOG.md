...
### Исправлено
- Повтор выпуска не обрывается молча, когда подготовка уже влита в мастер; разные категории фрагментов одной доработки больше не останавливают выпуск, а сведённый фрагмент сохраняет числа числами и остаётся пригодным для следующего выпуска. Повтор выпуска не обрывается молча, когда подготовка уже влита в мастер, а разные категории фрагментов одной доработки больше не останавливают выпуск: раздел выбирает свежая работа, и подготовка сообщает об этом. Провалившийся выпуск больше не блокирует следующий: подготовка сводит фрагменты пережившей его доработки в один файл и оставляет в журнале ровно одну запись, а брошенный кандидат получает отдельную отметку об отмене, поэтому повторный запуск проходит без ручной расчистки. (Фича 271, issue #7127, tasks ["T042", "T040", "T041", "T037", "T038", "T039"]); совместимость: Изменений совместимости нет.; release notes: Внутреннее улучшение процесса выпуска, для пользователя ничего не меняется.; ограничения: ["Продолжение выпуска после повтора с уже влитой подготовкой остаётся отдельной командой.", "\"Продолжение выпуска после такого повтора остаётся отдельной командой", "а не выполняется автоматически.\"", "Проверка на испорченный архив сохраняется: одинаковое имя файла при разных номерах доработки по-прежнему останавливает подготовку до изменений."]
