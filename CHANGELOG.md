...
### Добавлено
- В текущей ветке реализуется фича `049`: сохраненные итоги встречи для MVP.
  Веб-кабинет и встроенное окно macOS показывают краткое резюме, ключевые
  пункты, решения, действия, follow-up, риски, вопросы и ссылки на таймкоды
  только когда это подтверждается расшифровкой.
