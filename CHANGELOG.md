...
- Для релиза добавлены receipts в
  `specs/097-workspace-account-onboarding/validation/`: focused checks, полный
  PostgreSQL gate с четырьмя воркерами, review и traceability; PR
  [#3842](https://github.com/yshishenya/crisp/pull/3842) слит в
  `d79f24a9`.
...
- Миграции `0027_workspace_onboarding` и `0028_active_space_read` обратимы;
  выпуск не переносит legacy-записи или memberships. На текущем Docker
  allocation canonical CI использует четыре воркера; security scan 097 не
  входил в этот выпуск по прямому указанию пользователя.
