...
### Изменено
- В диалоге ручной загрузки клавиатурный фокус циклически остаётся внутри
  окна и возвращается к кнопке, открывшей его (PR #3891, T201, #3060).
...
### Операции
- Перед выпуском пройдены focused PostgreSQL-тесты (110), полный локальный
  CI (1937 параллельных и 34 строгих серверных теста, 583 macOS-теста),
  проверка синтаксиса и линтеры.
- Полный production Chrome/user-path receipt для старых задач T072/T085
  остаётся отдельной открытой проверкой; этот выпуск не объявляет её закрытой.
