...
- Review, timeline, exports и генерация итогов теперь используют одни и те же
  provider-attributed speaker turns; исходный ASR остаётся отдельным
  доказательным слоем без назначения спикера.

### Исправлено
- GRAF больше не назначает один overlap-winner всему ASR-сегменту и не считает
  `UNKNOWN` обычным подтверждённым участником. Некорректный provider result
  показывается один раз как неопределённый и получает явный degraded state.

### Безопасность
- Диагностика speaker-attribution ограничена версиями, счётчиками, статусами и
  hash результата без аудио, текста встречи, provider payload и signed URLs.
