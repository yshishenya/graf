...
- Перед изменением прав deploy проверяет, что группа secrets является приватной
  основной группой ровно одной локальной учётной записи. Файлы остаются
  недоступны остальным пользователям, а API, workers и операции обслуживания
  продолжают работать без root и с прежними read-only/capability-ограничениями.
