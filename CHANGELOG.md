...
<!-- Release features: F255 -->

### Добавлено
- _Пока нет записей._

### Изменено
- Спокойная матовая оболочка и компактные настройки с привычной навигацией GRAF (Фича 255, issue #6752, tasks [T010, T018, T019, T021, T022, T023, T024, T025]); совместимость: Миграций данных нет. Прежние левая навигация и меню пользователя сохранены; экспериментальная нативная замена отменена.; release notes: Добавлена общая скруглённая рамка содержимого окна, смягчены декоративные границы списка и настроек. На экране встречи убрана тяжёлая тень над плеером, декоративные разделители приведены к общей мягкой линии. Убраны одинаковые пустые значки в настройках автозаписи, уменьшены кнопки. По отзыву владельца восстановлены прежняя левая навигация и полное меню пользователя; удалён ставший ненужным механизм нативной замены меню. Подменю профиля открываются наведением по одному, имеют непрерывную зону перехода и закрываются при уходе указателя; убраны рамки отдельных вариантов темы. Выбор темы в профиле сохраняется без ухода с текущего экрана, с возвратом выбора при ошибке. Сохранены исправление жизненного цикла WebView и адресная чистка недостижимого кода. Не включать в публичный выпуск до закрытия обязательной приёмки.; ограничения: Матовая геометрия проходит отдельную приёмку; Liquid Glass и полная матрица остаются открытыми; Матрица macOS 14.5/26, доступности и совместных сценариев остаётся открытой
