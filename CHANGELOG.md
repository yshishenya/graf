...
- Навигация настроек теперь повторяет IA прототипа: отдельный «Обзор», четыре
  группы без дублирующей account-навигации и сохранение действий внутри нового
  settings-flow.
- Публичный лендинг переведён на editorial-композицию референса: новый hero,
  последовательность «Запись → Результат → Данные», локальные product proofs,
  адаптивное меню и сохранённые маршруты входа, скачивания и аналитики.
...
### Документы
- T084/T085 получили owner-attested product-market sign-off: зафиксированы
  канонический JTBD, результаты заявленных интервью, WTP/packaging review и
  границы, которые всё ещё блокируют public checkout.
- T087 закрыт: value hierarchy и контекстные non-coercive upgrade-сообщения
  зафиксированы для состояний Free, Trial и заполненного архива.
