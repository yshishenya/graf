...
### Безопасность
- Production-конфигурация снова отвергает пустой `auth_base_url`, чтобы OAuth callback URL не строился от недоверенного `Host` header при ошибочно пустой настройке.
