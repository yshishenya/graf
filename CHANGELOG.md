...
### Добавлено
- Feature `057-local-upload-custody`: серверный раздел `Записи встреч` теперь
  создает строку для активной отправки и показывает в ней статус `Отправляем`,
  процент и прогресс-бар с автообновлением списка.

### Изменено
- Feature `057-local-upload-custody`: активная отправка больше не выводится в
  правую нативную панель управления GRAF; пользовательский центр статусов
  отправки перенесен в серверный список записей встреч.
...
### Исправлено
- Feature `057-local-upload-custody`: валидные локальные записи больше не
  переводятся в ручной режим только из-за истекшего локального retention
  deadline, если файлы записи сохранены на Mac и пакет можно отправить.
  Автодоставка продолжает повторяться при временных сетевых/серверных ошибках.
- Feature `057-local-upload-custody`: очередь загрузки снова подхватывает
  сохраненные записи из legacy-каталога `2brain Rec/Recordings` после перехода
  приложения на каталог `GRAF/Recordings`, не перемещая пользовательские файлы
  и сохраняя проверку безопасной локальной очистки внутри доверенных каталогов.
- Feature `057-local-upload-custody`: чтение локальных manifest v2 стало
  обратно совместимым с новыми custody-полями, чтобы старые валидные записи
  не пропадали из автоматической очереди загрузки.
