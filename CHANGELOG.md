...
### Изменено
- Публичная главная страница стала короче и точнее: CTA `Начать` ведет во
  вход, шумный proof-row и подпись под кнопкой убраны, а блок совместимости
  заменен на движущуюся wordmark-полосу с примерами российских и глобальных
  сервисов для звонков без центрального бейджа.
...
- Публичная главная страница `rec.2brain.pro`: B2C-лендинг GRAF с прямым
  переходом в регистрацию, локальными product visuals и блоком готовых итогов
  встречи без demo/pilot flow.
