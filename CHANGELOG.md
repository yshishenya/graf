...
- Поиск, статус, доступ и сортировка остаются доступны в компактном embedded-
  ...
  соответствующие внутренние состояния.
- Поиск сопоставляется с названием, которое видно в строке встречи;
  ...
  переписываются эвристиками для системных записей и имён файлов.
...
- Выбор строк, удаление, фильтры и сортировка сохраняют keyboard focus и
  ...
  повторяет только строки с ошибкой.
...
- Нативный inspector показывает требующие реакции состояния локальной
  сохранности независимо от того, отвечает за следующий шаг владелец встречи,
  администратор, поддержка или политика хранения.
