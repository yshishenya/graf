...
### Изменено
- После подтверждения удаления запись сразу исчезает из списка без отдельного
  постоянного блока состояния; ошибки по-прежнему показываются в диалоге.

### Исправлено
- Убран лишний нижний баннер `Запись удалена из списка` / `Очистка данных GRAF
  продолжается`, который оставлял большое пустое пространство после удаления.
...
### Исправлено
- В диалоге экспорта убрана лишняя фраза о сохранении файла после удаления
  встречи; сведения о границах удаления остаются в разделе сведений и отчёте.
...
### Исправлено
- Исправлен HTTP 500 на странице встречи после AI-генерации итогов: сервер
  нормализует `evidence_kind` у новых source refs и безопасно читает ранее
  сохранённые refs без этого поля.
...
### Документы
- Добавлена metadata-only live receipt для комбинированного T057-прогона:
  forced crash, gated promotion и rollback подтверждены без вывода текста
  транскрипта в репозиторий.
...
- Judge verdict теперь детерминированно выводится из числового score (`>= 0.5`);
  исходные несовпадающие score/verdict остаются в полном raw trace, а held-out
  gate `0.9` не смягчается.
...
### Добавлено
- Компактное меню «Ещё» на странице встречи с быстрым экспортом, скачиванием
  аудио, сведениями о встрече и безопасным удалением.

### Изменено
- Служебные файлы, ревизия, календарный контекст, спикеры, активность и границы
  удаления вынесены из быстрого меню в отдельное окно сведений.

### Исправлено
- Убрана перегруженная модальная панель действий; добавлены предсказуемая
  клавиатурная навигация, возврат фокуса и адаптивная раскладка меню.
...
- Feature 121 / T096 / #4253: выбор формата итогов больше не откатывается в
  ...
  остаётся кандидатом до явного принятия.
- Content export теперь fail-closed при смешении нового транскрипта со старым
  итогом: интерфейс сохраняет CAS-указатель принятой версии, но экспорт не
  выдаёт неполный или смешанный файл.

### Безопасность
- _Пока нет записей._

### Документы
- Добавлены metadata-only regression evidence для UI → API → Temporal,
  миграции `0032_outcome_pointer`, конкурентного удаления и stale export.
