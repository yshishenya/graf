...
### Безопасность
- Email-code login теперь привязывает проверочный код к нормализованному email, для которого был начат вход, чтобы код из одного login state нельзя было обменять на сессию другого активного пользователя workspace.
