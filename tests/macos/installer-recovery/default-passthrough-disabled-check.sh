...
if ! printf '%s\n' "$NEW_LOG" | rg -q 'passthrough_bridge_armed|passthrough_bridge_started|passthrough_bridge_already_active'; then
  echo "default-passthrough-disabled-check: expected automatic non-recording route evidence during default app launch" >&2
  exit 1
fi

if printf '%s\n' "$NEW_LOG" | rg -q 'passthrough_bridge_started' &&
...
if printf '%s\n' "$NEW_LOG" | rg -q 'passthrough_bridge_armed' &&
   ! printf '%s\n' "$NEW_LOG" | rg -q 'waiting for virtual device client'; then
