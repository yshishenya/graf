...
let expectedName = "2brain Rec Microphone"
let devices = try allAudioDevices()

print("Synthetic mic route preflight:")
for device in devices {
    print("- \(device.name) input=\(device.inputChannels) output=\(device.outputChannels)")
}

guard let virtualMic = devices.first(where: { $0.name == expectedName }) else {
    fail("Expected \(expectedName) to be visible to Core Audio.")
}

guard virtualMic.inputChannels > 0 else {
    fail("\(expectedName) is visible but has no input channels.")
}

guard virtualMic.outputChannels == 0 else {
    fail("\(expectedName) must not expose output channels; remote audio must not route into the mic path.")
}

print("Synthetic mic route preflight: ACCEPTED")
