...
def test_portable_event_identity_defers_real_merge_group_mapping() -> None:
    result = ci_contracts.resolve_event_identity(
        {"merge_group": {"head_sha": "a" * 40, "base_sha": "b" * 40}},
        "merge_group",
    )
...
def test_app_snapshot_preserves_bundle_symlinks(monkeypatch, tmp_path):
    ...
    target = contents / "Versions" / "A"
    target.mkdir(parents=True)
    link = contents / "Current"
    link.symlink_to("Versions/A", target_is_directory=True)
    monkeypatch.setenv("GRAF_DEV_INSTALL_PATH", str(destination))

    backup = adapter._snapshot_app()

    assert backup is not None
    assert (backup / "Contents/Current").is_symlink()
