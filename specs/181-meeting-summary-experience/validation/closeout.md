...
- 2026-08-30 dispatch-race correction: Feature 181 focused quickstart — `105 passed`, 2 warnings; dedicated dispatch regressions — `4 passed`; `infra/scripts/ci-local.sh --fast` — `1346 passed`, lint и Python compile pass. Durable Temporal start acknowledgement loss теперь не показывает ложный error, не отменяет потенциально принятый deterministic workflow и возвращает async `202` с сохранением текущих итогов.
