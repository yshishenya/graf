...
- **FR-009**: Focused validation MUST fail closed при потере `converge`, неправильном порядке стадий, decoy-последовательности вне канонической workflow-секции, потере reviewer ownership, project-local skills или поддерживаемой lock schema.
...
- **SC-004**: Focused validation detects 100% of the six protected failure classes: missing convergence stage, wrong workflow order, decoy sequence outside canonical workflow sections, missing reviewer ownership, missing project-local skills and unsupported lock schema.
...
- Published `speckit-bootstrap v0.9.7` и `github-issue-canon v0.3.2` являются stable fail-closed tooling baseline.
