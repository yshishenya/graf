# Quickstart: MVP Owner Journey Proof
...
```sh
SPECIFY_FEATURE_DIRECTORY=specs/051-mvp-owner-journey-proof \
  .specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks
```

Expected: JSON points to `specs/051-mvp-owner-journey-proof`.
...
Expected: all tests pass after 051 updates.

## 3. Production Owner Journey Probe

```sh
python3 specs/051-mvp-owner-journey-proof/evidence/production-owner-journey-probe.py
```
...
NODE_PATH="${CODEX_NODE_MODULES:-node_modules}" \
  "${CODEX_NODE_BIN:-node}" \
  specs/051-mvp-owner-journey-proof/evidence/browser-runtime-check.cjs
```

Expected: web desktop, web mobile-width, embedded desktop, and embedded
mobile-width review checks report `failures=[]`, no horizontal overflow, no
blocking console errors, visible speaker lanes, visible outcomes states, and
working timestamp seek.
...
verification passes for the local build, and active process state is recorded
without interrupting a recording.

## 7. Production Health And Deployment Truth
...
rg -n -i \
  ...
  specs/051-mvp-owner-journey-proof docs/evidence/051-mvp-owner-journey-proof docs/current-product-status.md CHANGELOG.md
```
