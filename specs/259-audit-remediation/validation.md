...
## Возврат к трёхзначной нумерации — 2026-09-11

- Lane: active-spec-kit / significant governance, продолжение US3/B019.
- База совпадает с проверенным origin/master: ad71f2ce4db68d846d7c333213961c5f5f7d5e89.
- До ремонта offline suggestion =6795. Политика исключала только6788,
  а specs6791/6792 снова поднимали sequence. `%03d` задавал минимальную
  ширину, не верхнюю границу. Generated specify независимо считал max(specs)
  и мог заменять полный pointer объектом только с feature_directory.
- Первые четыре регрессии упали на исходном коде: каскад historical IDs,
  исчерпание диапазона и два explicit claim обхода.
- Требования FR-027–029/SC-008–009 проверены независимо; см.
  checklists/three-digit-numbering.md. Авторский analyze: требований без
  задач нет, блокирующих конфликтов с constitution нет; старое допущение
  F1000+ явно заменено текущим решением владельца только для GRAF.
- После исправления read-only online suggestion: next_available264,
  occupied_count238, mode=github-checked. Режим не изменил refs, shared claims
  или active pointer (сравнение содержимого до/после). Номер не резервировался.
- Self-test распределителя PASS; GRAF governance invariants без frozen doctor
  PASS. Это локальные проверки, не GitHub exact-SHA CI и не полный frozen doctor.
- Точный повтор исторического claim и draft upgrade сохраняются, новые большие
  claims запрещены. Source overlay обновлён без установки глобального bootstrap.
- Исторический Issue #6837 CLOSED; он не переоткрывался и новые комментарии
  не отправлялись. Этот ремонт локален; commit, новый PR/issue sync,
  governance-fast и включение в master остаются отдельными этапами.

Финальные результаты локальной проверки:

- `pytest tests/governance/test_feature_allocator.py tests/governance/test_feature_identity_guidance.py tests/governance/test_validator_safety.py -q`: **161 passed, 15 skipped** за10.49s. Все skips — отсутствующий pwsh.
- Source bootstrap: **29 unit tests passed**; 14 исходных преобразований
  применены к пяти файлам из HEAD и повторно: точное совпадение с установленными
  файлами, повторное применение ничего не меняет. Существовавшие изменения
  исходного bootstrap сохранены. Полный refresh и глобальная установка не выполнялись.
- Изменённые Python/Bash scripts проходят проверку синтаксиса; `git diff --check`
  PASS в обоих репозиториях. В lock обновлены только tree hashes git extension
  и speckit-specify; версии других компонентов не изменены.
- Ponytail review: дополнительных зависимостей, счётчиков или сервисов нет;
  проверки используют существующий allocator и штатные оболочки. Удалять
  обязательные проверки согласованности и сохранения истории оснований нет.
- Converge текущего этапа: FR-027/028/029, SC-008/009 и T020–T023 реализованы
  и локально проверены, дополнительных обязательных задач реализации нет.
  Это не завершает общий продуктовый бэклог F259. Commit/PR/CI/merge и
  применение исправления в новых рабочих копиях из master ещё не выполнены.

### Подготовка PR по поручению владельца — 2026-09-11

Пользователь разрешил коммиты и публикацию PR сообщением «готовь PR к релизу».
Текущий ремонт привязан к отдельному Issue #6938 (T020–T023), umbrella #6827
остаётся открытым. Исходные преобразования перенесены на свежий main bootstrap
074681076a21183015a3ecdc796b2f5240ada8aa в отдельной временной рабочей копии;
посторонние правки старой копии не включаются. Проверены реальные преобразования
на generic исходниках и прежних состояниях: общий номер, обязательная политика,
резервирование, запрет неизвестного состояния, повторное применение без изменений.
Pinned smoke с Spec Kit v1.0.1 / issue-canon v0.3.2: PASS, включая чистую
установку, повторное обновление, integrity probes и frozen doctor.
Это подготовка изменений к включению в релиз; merge, тег и deployment не выполняются.

Проверки непосредственно перед коммитом PR:
- GRAF: 161 passed, 15 skipped за11.77s; пропуски только PowerShell.
- Bootstrap на актуальном main: ci-local PASS (33 unit tests, ShellCheck,
  actionlint, zizmor); расширенная проверка GRAF/numbering overlays PASS.
- Отдельный source commit: 49f061a (полный SHA и remote CI фиксируются в PR).
- Предрелизная проверка diff: PASS. Выпуск/установка/production не выполнялись.
