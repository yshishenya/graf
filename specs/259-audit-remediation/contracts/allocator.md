...
## Действующий диапазон GRAF — 2026-09-11

Предыдущее утверждение об отсутствии предела999 заменено по FR-027–029.
Политика допускает дополнительный max_feature_id: положительный JSON integer
(не bool); в GRAF 999. Seed использует только spec IDs <= max_feature_id
за вычетом исторических исключений. Занятость не фильтруется. Каждый следующий
кандидат ограничен сверху; исчерпание приводит к ошибке без резервирования.
Отсутствующая граница сохраняет прежний общий контракт. Новый explicit claim
вне диапазона отклоняется; точный прежний claim и draft upgrade сохраняются.
Номер в новой ветке имеет каноническую ширину 3. Ветка, каталог и указатель
должны разделять зарезервированный номер; указатель обновляется без потери полей.
