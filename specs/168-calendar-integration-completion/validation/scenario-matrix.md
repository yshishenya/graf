...
| S168-21 Google revoke/429/410 | reconnect/backoff/full sync | same | safe state | PASS-SYNTHETIC: adapter mappings, bounded 429 retry, revoked credential invalidation and disposable-PostgreSQL `old cursor → 410 → full sync → new cursor` replacement pass; live Google revoke/429/410 remain NOT-PROVEN |
...
| S168-37 public Google Calendar data use | Open the homepage FAQ, review the expanded Google Calendar answer and follow the privacy link | public responsive page | Explain selected-calendar reads, upcoming/reminder/context purpose, read-only limits, disconnect control and no automatic recording/join without claiming public Google approval | PASS-SOURCE + PASS-SYNTHETIC (16/16 public landing tests) + OBSERVED-RUNTIME at 1440x1000 and 390x844; first FAQ item expanded, no duplicate calendar section/navigation, zero overflow, mobile menu intact, native toggle and privacy navigation pass, contrast ≥ 5.12:1, no browser warnings/errors; local only. Live homepage returned 200 without the FAQ, so T072 remains open |
