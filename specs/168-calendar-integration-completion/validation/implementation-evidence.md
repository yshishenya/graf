...
| Provider truth | Google, Yandex, Mail.ru and CalDAV aliases expose working actions; EWS and Bitrix24 remain visible but non-interactive. | OBSERVED-RUNTIME + PASS-SOURCE; unavailable adapters are not advertised as connectable. |
...
| Google domain ownership and branding remediation | Search Console confirmed DNS ownership of `2brain.pro`; the ownership issue disappeared from Google branding review. An authorized re-verification submission reached `Verification in progress…` and then returned the two remaining homepage-purpose/name findings. Google’s official remediation guide requires the homepage to explain both the app purpose and how requested Google user data is used; the previous public page described recording/results but not calendar-data use. The current worktree now adds a visible `Как GRAF использует Google Calendar` section with read-only purpose, no hidden start and user-control truth; the local page was visually inspected and the focused public-page/analytics selection passes 29/29. | OBSERVED-RUNTIME + PASS-SOURCE/PASS-SYNTHETIC; keep the DNS verification TXT record. The landing correction is not deployed and branding must not be resubmitted until the public URL shows it. No DNS value, account identity or credential was retained. |
