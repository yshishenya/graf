...
Страница: два компактных индикатора, один раскрытый шаг с пользой для записи разговора и одним главным действием. Готовность не закрывает страницу. Выход закреплён вне прокручиваемой части. Settings guide содержит путь, имя приложения, неинтерактивную иллюстрацию и пояснение перезапуска. Технические способы восстановления раскрываются по запросу. Restricted предлагает вернуться в GRAF и обратиться к администратору. Busy различает ожидание запроса и проверку. Вход в настройку остаётся доступным при missing permission даже во время ожидающего запроса; busy запрещает запись и дублирующий запрос.
...
## Уточнение UI: пользовательская задача (T008)

Переиспользовать тот же DesktopPermissionOnboardingView и callbacks. Верх — компактный прогресс Ваш голос/Собеседники; центр — цель текущего шага и одна короткая причина доступа. Системная инструкция появляется в контексте отказа/перехода в Settings. Основное действие закреплено рядом с «Настроить позже»; повторная проверка/перезапуск/ручной запрос доступны в раскрываемой помощи. Состояния ожидания запроса и проверки различаются дополнительным isChecking, без изменения сервиса разрешений. Restricted объясняет администратора и предлагает вернуться в GRAF. Никаких новых dependencies, авторазрешений, анимаций, тестового захвата или новых экранов. Native screenshots всех состояний + существующие guards/tests/Quit proof + exact-SHA CI.
