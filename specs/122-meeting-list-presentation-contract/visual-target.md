...
| Количество результатов | добавить контекстно | `Найдено: N`, либо `Найдено: больше N` при усечённой выдаче |
...
| Refined result count | `Найдено: N`; при наличии следующей ограниченной страницы — `Найдено: больше N` |
...
|---|---|---|---|
| Открыть | click на читаемую область/title | `Enter` на основной ссылке | `Открыть встречу …` |
| Выбрать | checkbox | нативный `Space` на checkbox | `Выбрать встречу …` / selected state |
...
| Нужно заново выбрать desktop-пространство | `Нужно выбрать пространство` | `Доступ к выбранному пространству больше не подтверждён.` | `Войти и выбрать пространство` |
| Доступ к списку запрещён | `Нет доступа к встречам` | `Обратитесь к владельцу рабочего пространства.` | отсутствует |
| Доступ к отдельной встрече отозван | `Встреча больше недоступна` | приватные metadata не повторяются | вернуться к списку |
| Deletion accepted | видимый status отсутствует | скрытое объявление `Запись удалена из списка.` | отсутствует |
...
Нефокусируемый `li` не дублирует основную ссылку. Checkbox, основная ссылка, отдельное календарное действие и delete остаются нативными точками фокуса; визуально тихие controls раскрываются через `focus-within`, но не удаляются из accessibility/tab sequence. Status и result-count announcements не перемещают focus. После исчезновения focused control фокус возвращается к основной ссылке следующей доступной строки, предыдущей строки либо заголовку списка в этом порядке; более новое перемещение пользователя имеет приоритет.
...
На ширине меньше поддерживаемого desktop target полная правая подпись времени переносится под content-колонку. Она не сокращается до неоднозначной даты и не создаёт горизонтальную прокрутку.
...
13. loading;
14. offline/unavailable/session/workspace-reselection/access-denied;
