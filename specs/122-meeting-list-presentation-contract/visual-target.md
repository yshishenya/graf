...
`Сначала новые` является default. При `Недавно обновлённые` правая подпись имеет форму `Обновлено 21 июл, 19:22`; во всех остальных режимах она описывает саму встречу: `21 июл, 19:22` либо `Без даты`.
...
| 2 | результат не удалось подготовить | `Не удалось обработать` | существующий recovery path |
