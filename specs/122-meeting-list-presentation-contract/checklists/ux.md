...
- [x] CHK009 Is result count visibility limited to active search/filter refinement, with exact `Найдено: N` copy and truthful `Найдено: больше N` disclosure for a truncated bounded response? [FR-004; Visual Target §Toolbar Copy]
...
- [x] CHK047 Are visually quiet contextual controls required to remain in the tab order and accessibility tree, with hover/focus/selection/coarse-pointer reveal preserving the same actions? [FR-018; UI Contract §Contextual controls]
...
- [x] CHK049 Is metadata suppression after session expiry, workspace reselection, and access denial/revocation explicit and comprehensive, with only the applicable recovery action? [FR-025; Visual Target §Empty, Loading And Recovery Copy]
...
- [x] CHK051 Does evidence cover every canonical row/list class, including distinct workspace-reselection and access-denied recovery states, rather than only a happy path? [SC-011; Visual Target §Evidence Matrix]
