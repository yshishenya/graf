...
**Decision**: Keep fixed intent and delete target columns. Pointer hover, keyboard focus-within, or selected state reveals controls without changing column geometry. The native checkbox and button remain in the logical tab/accessibility sequence even while visually quiet; receiving keyboard focus reveals the row before the control is painted as focused. They are never suppressed with `aria-hidden` or `tabindex=-1`. Non-hover/coarse-pointer media keeps them persistently visible. Each contextual target is at least 32×32 CSS px.
...
**Decision**: Remove the duplicate `Записи встреч` heading and persistent sort subtitle. Show `Найдено: N` only when search or filters are active, or `Найдено: больше N` when the bounded response proves that the visible page is truncated. Place deletion feedback above the list and keep it in one polite/atomic live region. Announce intentional refinement, bounded progress changes, deletion, and error results without moving focus; do not re-announce an unchanged count during one-second progress polling. After a focused control disappears, restore focus to the primary link of the next row, previous row, recovery action, or list heading unless the user has already moved elsewhere.
