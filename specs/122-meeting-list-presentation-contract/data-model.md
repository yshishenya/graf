...
| `status` | Canonical web transport remains `ready`, `processing`, `partial`, or `failed`; legacy internal URL values normalize into one visible group before validation. |
...
| result count | Render `Найдено: N` only while refinement is active; use `Найдено: больше N` when the bounded response proves more matches exist. |
...
- Reset clears only query, status, and access in one action; it preserves the selected sort in both the HTMX interaction and no-JavaScript fallback URL.
...
| `surface_state` | enum | `content`, `first_empty`, `no_results`, `loading`, `offline`, `unavailable`, `session_expired`, `workspace_reselection`, or `access_denied`. |
...
- Session, workspace-reselection, and access-denied projections contain no cached meeting title, participants, calendar context, or other private meeting metadata; only workspace reselection exposes the bounded `Войти и выбрать пространство` action, while access denial has no action.
