...
**Release Gate**: The implementation lane completed before deployment. The user subsequently authorized product commits, repeated review/fix loops, PR closeout, release, installed-app verification, and production rollout. Those actions proceed only after a clean complete-diff review, master synchronization, full repository validation, and the release/deploy gates in `docs/agent-guidance/release-and-validation.md`.
...
**Scale/Scope**: One meeting-list route rendered for browser and embedded variants; one toolbar; one row pattern; one client-side selection mode; 11 canonical row states and 16 evidence classes. Meeting detail, transcript, notes, playback timeline, native capture rail, onboarding, settings, and new organization features are outside product scope. Production deployment is a separate authorized release lane and does not expand the feature behavior.
...
8. Run `infra/scripts/ci-local.sh`. Hand the validated implementation to the separately authorized release lane; deploy or refresh the installed app only after its own gates pass.
