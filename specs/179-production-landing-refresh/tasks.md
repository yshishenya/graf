...
- [ ] T049 Create the production app CalVer tag `vYYYY.MM.DD.N`, publish the matching GitHub Release, and add Russian plain-language release notes in `docs/releases/`; leave all three artifacts unchecked until they exist and are verified against the reviewed SHA
- [ ] T050 Record the owner's 2026-08-21 confirmation of external legal approval for immediate Metrica in `specs/179-production-landing-refresh/validation/operational-approvals.md`, and separately record every required billing canary/four-eyes gate; legal approval alone MUST NOT satisfy billing or production-release gates
