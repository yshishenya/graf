...
- Первый публичный каталог содержит `Free` и `Личный`; enterprise/team sales-assisted остаётся вне self-service. Утверждённая базовая цена запуска для `Личный`: 790 ₽/month или 7 900 ₽/year + 2 GB. `Free` = 300 server-processing minutes per calendar month, reset at day 1 `00:00 Europe/Moscow` with no rollover, + 250 MB playback archive; Trial = 500 MB. One co-termed add-on selects total capacity 5/20/100/500 GB with approved monthly prices 290/790/1 990/4 990 ₽ and annual prices 2 900/7 900/19 900/49 900 ₽ respectively. Decimal units are authoritative (`1 MB = 1 000 000 bytes`, `1 GB = 1 000 000 000 bytes`). Checkout remains default-off until YooKassa, unit-economics, finance, accounting, legal and release gates are complete.
...
| `Storage add-on` | does not change feature/usage entitlement | total 5/20/100/500 GB | 290/790/1 990/4 990 ₽ monthly; 2 900/7 900/19 900/49 900 ₽ annual | расширить archive without inventing a duplicate paid tier |
...
The pricing page MUST compare `Free`, `Trial Личного` and `Личный` without presenting add-on as a separate base plan. Storage add-on options and approved prices are shown only as an optional extension of `Личный`; annual prices are labeled `2 месяца бесплатно` while they remain exactly ten monthly prices. Changing an approved base or add-on price creates a new catalog/offer version and re-runs checkout, receipt, COGS and usability gates.
...
- Q: Какую базовую цену утвердить для тарифа `Личный`? → A: 790 ₽/месяц или 7 900 ₽/год. Цена storage add-on и production checkout остаются отдельными approval/release gates.
- Q: Как показывать storage add-on после утверждения его цены? → A: Показывать его как необязательное расширение тарифа `Личный` с итоговой ценой и без отдельного базового тарифа.
- Q: Какие цены утвердить для storage add-on? → A: Премиальная лестница: 5 ГБ — 290 ₽/мес или 2 900 ₽/год; 20 ГБ — 790 ₽/мес или 7 900 ₽/год; 100 ГБ — 1 990 ₽/мес или 19 900 ₽/год; 500 ГБ — 4 990 ₽/мес или 49 900 ₽/год.
- Q: Кто является продавцом для YooKassa? → A: ИП на УСН. Конкретный объект УСН, ставка, чековая конфигурация и налоговая разметка требуют подтверждения бухгалтера до production enablement.
- Q: Какие циклы оплаты включить в первый запуск? → A: Месячная и годовая подписка одновременно: 790 ₽/месяц и 7 900 ₽/год.
- Q: Как показывать цену пользователю? → A: 790 ₽/месяц и 7 900 ₽/год являются итоговой суммой без неожиданных доплат; налоговая разметка чека подтверждается бухгалтером отдельно.
- Q: Какой объект УСН у продавца? → A: «Доходы»; рабочая ставка — 6%, но её и чековую конфигурацию нужно подтвердить по действующему налоговому статусу ИП перед production enablement.
- Q: Какой статус НДС использовать в чеке? → A: «Без НДС»; настройку нужно сверить с магазином YooKassa и актуальным налоговым статусом до production enablement.
- Q: Какой статус YooKassa shop доступен сейчас? → A: Магазин оформлен на продавца, тестовый режим доступен; production checkout и реальные списания остаются выключенными до canary и внешнего sign-off.
...
- **FR-021**: Initial public self-service catalog MUST содержать только `Free` и `Личный`; `Личный` MUST иметь monthly cycle 790 ₽ и annual cycle 7 900 ₽ одновременно. Team/enterprise MUST отображаться как отдельный sales-assisted path без self-service charge.
...
- **FR-023**: Утверждённая базовая цена `Личный` MUST быть 790 ₽/month или 7 900 ₽/year и показываться как итоговая сумма без неожиданных доплат. Storage add-on MUST использовать утверждённую лестницу: 5 ГБ — 290/2 900 ₽, 20 ГБ — 790/7 900 ₽, 100 ГБ — 1 990/19 900 ₽, 500 ГБ — 4 990/49 900 ₽ для month/year соответственно. Provider minimum floor, integer-minor-unit rounding, VAT/receipt mapping и offer version MUST быть подтверждены product/finance/legal owners до production enablement и опубликованы как единая server-owned truth для pricing page, checkout, account и receipts; payable total MUST быть положительным и не ниже floor.
...
- GRAF запускается Russia-first, рублёвый self-service payment идёт через один YooKassa shop продавца — ИП на УСН с объектом «Доходы»; рабочая ставка — 6%, чековый статус — «Без НДС». Эти настройки должны быть сверены с магазином YooKassa и актуальными данными ИП до production enablement.
...
- Первичная коммерческая модель: `Free` + один paid `Личный`, explicit once-per-identity 7-day cardless trial, monthly/annual cycle, unlimited paid meetings/transcription/AI and separate playback archive capacity. Утверждённая базовая цена: `Личный` 790 ₽/month or 7 900 ₽/year + 2 GB. Free 300 minutes/month + 250 MB, Trial 500 MB и total-capacity add-ons 5/20/100/500 GB с утверждённой ценовой лестницей сохраняются как модель; checkout stays off until remaining YooKassa, unit-economics, finance, accounting, legal and release gates are resolved.
