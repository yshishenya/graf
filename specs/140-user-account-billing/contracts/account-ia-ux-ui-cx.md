...
| Payment return `/billing/checkout/status/{safe_number}` | one operation timeline | `Проверить статус`, `Вернуться в кабинет`, `Добавить способ оплаты` when method_required, `Попробовать снова` only after final canceled | redirect received, processing, success, canceled, unknown, method required; never imply success early |
...
| Referrals `/account/referrals` (implementation aliases `/referrals`, `/account/referrals`) | personal-workspace opaque `/r/{opaque}` link, invitee discount, `+7 дней за месяц / +30 дней за год`, 180-day rolling cap, paid/bonus-until, masked status/time-credit ledger | `Активировать ссылку` once, then `Скопировать ссылку`, `Поделиться`, `Как это работает`, `Обратиться в поддержку` | unissued/unavailable, active, no campaign/attributed/pending/available/applied/expired/rejected/reversed; one stable link supports many invitees; Free credit waiting up to 12 months |
| Referral landing `/r/{opaque}` → auth CTA | inviter benefit without identity disclosure, referee first-period discount, terms/expiry/privacy; token is stored only in a secure first-touch cookie after an active DB lookup | `Создать аккаунт`, `Войти`, `Продолжить без бонуса` | valid, expired/invalid, dependency unavailable, existing account (no re-attribution), already attributed, first-touch stored, conflicting promo later shows best one |
