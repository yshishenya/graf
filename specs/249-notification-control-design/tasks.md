...
## Phase 18: Convergence — сохраняющий данные переход Dev

- [ ] T038 Добавить и независимо проверить сохраняющий данные переход схемы единственного GRAF Dev по `dev-schema-transition.md`: доказанный предок, неизменные образы хранилищ, согласованный холодный снимок PostgreSQL/MinIO, компенсация до допуска записи данных, отказ от автоматического возврата старого снимка после запуска worker. Область: `scripts/dev-harness.py`, `infra/scripts/dev-migration-preflight.py`, `tests/governance/`, `docs/agent-guidance/local-development.md`. Текущий отказ не обходить. [FR-015/018; T022/T030; live schema0086→0088 blocker; HIGH]

T038: https://github.com/yshishenya/graf/issues/6786


## Изменение приёмки владельцем — 2026-09-07

Владелец явно исключил проверку VoiceOver из текущей приёмки PR. Этот пункт
отмечается как исключённый по указанию владельца, а не PASS. Доступные имена
элементов, клавиатура, фокус, масштаб и видимость Stop остаются требованиями.
Остальные критерии и защита данных не ослаблены.

## Phase 19: Convergence — нативный отклик

- [X] T039 Устранить блокирующую анимацию `NSPopover` в `CalendarTrayController`, сохранив bounded-геометрию и существующие действия; подтвердить установленным GRAF Dev p95 открытия панели ≤200 мс и отсутствие активной записи после замера. [SC-006; FR-013; partial]

T039: исправление проверено на `6fefaeb5b68adcd4fd30ca0d5f447b9114891398`:
штатные build/promote/status/smoke 13/13 PASS, реальные значения
`presentation_ms` n=8 дали nearest-rank p95 134.666500 мс. Оставшиеся
аппаратные ограничения перечислены в `implementation-evidence.md`; новых
неподтверждённых задач по коду при convergence не добавлено.

## Совместное релизное ревью F239/F249/F255

- [X] T040 Сохранить тихие напоминания о других календарных встречах во время записи, подавлять только явно связанную встречу; показывать объяснение и восстановление HTML-формы при 409/422 в обычном и встроенном кабинете. Владелец — существующий #6635; PR #6790; проверки DesktopNotificationControlTests и test_notification_inbox_routes.py.

- [X] T041 Исправить совместимость уведомлений со Swift 6.0.3: переносить только статус разрешения, пометить экран настроек главным актором. Владелец — #6635; 48 целевых тестов PASS; отдельная macOS-диагностика и полный CI нового кандидата остаются релизными проверками.

- [X] T042 Согласовать OpenAPI уведомлений, перечень RLS-миграций и документ политики server_notifications; отделить заголовки панели от основного содержания в тесте подключения профиля, явно задать вежливое озвучивание статуса уведомлений. Владелец — #6635. Целевые серверные проверки: 111 PASS; полный CI нового кандидата остаётся отдельным релизным gate.
