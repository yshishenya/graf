...
- [X] T021 [US5] Проверить объединённые server/Node/Swift, media role и первый embedded вход, автоматическое появление итогов с сохранением player/draft в единственном GRAF Dev после F257; evidence в specs/6788-macos-notification-recovery/validation.md. FR010–014, SC006–008; installed часть вместе с T014, до T015.
...
## Phase 13: Приёмка общего релиза

- [X] T028 [US5] Исправить откладывание обновления итогов при открытом редакторе названия в apps/server/src/twobrain_rec_server/cabinet/static/cabinet/cabinet.js; сохранить живой черновик, фокус, проигрывание и работоспособность сохранения/отмены после обновления. Добавить исполняемую browser-регрессию, проверить Chromium/WebKit и повторить установленный сценарий >5мин pending → публикация ≤15с. FR012/SC007 (contradicts, HIGH), #6839: на общем SHA63b9b58 результат не появился за20с из-за titleEditorActive guard; T014/T021/T015 остаются открыты.

- [X] T029 [US1/US2] Исправить выбор окна в AppLifecycleDelegate (apps/macos/RecApp/App/TwoBrainRecApp.swift): после действия уведомления повторная активация/reopen сохраняет нативную вкладку на переднем плане; штатные команды записи не открывают скрытый кабинет. Сохранить явные входы и восстановление неготового handler; добавить исполняемую регрессию и повторить установленную карточку/скрытый кабинет. FR002/FR005, #6840: на SHA63b9b58 после карточки settings→reopen_visible перекрывает выбранное окно.

- [ ] T030 [US2] Исправить контекстный вход «Автоопределение встреч» в apps/macos/RecApp/App/TwoBrainRecApp.swift на существующую нативную вкладку автозаписи; выполнить регрессию настоящего callback в AppLifecycleWindowRegressionTests.swift, отрицательную проверку старого кода и установленное AX-нажатие после обновления Dev. FR-003/FR-004, независимый review; неверный openSettings(nil) направлял в общий кабинет. (Issue #6841)
