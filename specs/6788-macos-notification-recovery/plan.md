...
общим стендом. Для совместимости со схемой0090 ветка перенесена поверх
F256/PR #6828; F256 сохраняется отдельной зависимостью, её изменения не
объявляются реализацией F6788. Понижение и подмена manifest запрещены.
После готовности зависимости использовать штатный переход harness. Отчёт разделяет unit, сборку,
