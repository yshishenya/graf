...
Повторить F214 с компактной карточкой:320×192pt, цельный текст действий, светлая/
тёмная тема, положение под значком/внутри выбранного экрана. КлавиатураEnter/Escape
и remember; после решения/окончания встречи0 постоянных окон. Геометрия маленького
и второго экрана — исполняемый unit; реальный multi-display только если доступен.

- До клика на карточке frontmost app/key window неизменны; Enter/Escape в
  Zoom не разрешают/не отклоняют запрос. После явного клика Tab/Space меняют
  remember, Enter/ Escape выполняют Record/Skip. Таймер не перезапускается.
- При 240×160pt вся карточка внутри visibleFrame, действия и remember доступны
  прокруткой, кнопки вертикальны; длинное имя переносится, disabled-текст
  читаем без уменьшения шрифта. Проверить реальное содержимое, не только CGRect.
- Якорь только на выбранном экране при видимом меню; hidden/no/foreign anchor
  дают top-right fallback. Геометрия покрывает отрицательные origin и повторный
  layout; fullscreen проверяется установленным Dev, физический второй экран
  отмечается отдельно при недоступности.
