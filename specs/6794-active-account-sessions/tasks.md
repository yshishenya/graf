...
- [X] T001 [US1] Расширить регрессию только действующих входов, unknown, одинаковых имён, пустого и недоступного состояния в `apps/server/tests/contract/test_account_routes.py` (FR-001–004, FR-007–008). (Issue #6926)
- [X] T002 [US1] Убрать историю и уточнить основной список, область пространства и справку в `apps/server/src/twobrain_rec_server/cabinet/templates/cabinet/pages/settings_account_content.html` (FR-001–004, FR-007–009). (Issue #6927)
...
- [X] T003 [US2] Согласовать подписи одиночного/массового завершения в шаблоне и `apps/server/src/twobrain_rec_server/cabinet/web_routes/settings.py`; проверить их в `apps/server/tests/contract/test_account_routes.py` и существующем `apps/server/tests/integration/test_account_lifecycle.py` (FR-005–006, FR-008). (Issue #6928)
## Phase 4 — Validation and closeout
- [X] T004 Проверить `quickstart.md`, записать локальное evidence/converge в `specs/6794-active-account-sessions/validation.md` и фрагмент `changes/unreleased/F6794.yaml`; до закрытия issues получить review и governance-fast exact PR SHA (SC-001–004). (Issue #6929)
...
## Подготовка PR — актуальный результат
T001–T005 выполнены. PR #6942, governance-fast PASS на исходном коммите 0a54cf79a6e6a4418f42ac6212d26e7795208c5a: https://github.com/yshishenya/graf/actions/runs/34609124722. Последующее добавление evidence и снимков требует повторного GitHub check финального SHA, публикуемого в PR. Issues сохраняются открытыми для установленного closeout после слияния/релизных gates; выполненные задачи не означают публикацию.
