...
## Интеграция выпуска v2026.09.13.1

После объединения F211 A3 быстрый CI впервые включил существующую проверку общих кнопок F191: она ожидала старое «Выйти» для чужого входа. Проверка приведена к принятой F6794 подписи «Завершить»; проверки геометрии и запрет старого «Завершить сеанс» сохранены. Продуктовый код не менялся.

`GRAF_CI_BASE_REF=origin/master infra/scripts/ci-local.sh --focused`: PASS, 112 tests, 0 skipped, pytest 2.29 s; выбранные проверки оболочки, настроек и моделей. Это локальная диагностика; слияние требует нового GitHub governance-fast на итоговом SHA. Lane: active Spec Kit, исправление устаревшей проверки при интеграции релиза.
