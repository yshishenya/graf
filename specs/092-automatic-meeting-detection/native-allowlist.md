...
1. Parse Control Center `sensor-indicators` attribution events.
2. Keep only `mic:<bundle_id>` entries whose bundle ID is in this native meeting
   allowlist.
3. Ignore every non-allowlisted `mic:` / `aud:` / `cam:` attribution.
...
Browsers are not part of this native mic-attribution allowlist. Browser meetings
must use the browser metadata path: active tab URL/title/service family plus
calendar or join intent, with browser microphone attribution only as supporting
evidence.
...
| `diagnostic_only` | Bundle ID is known, but live mic attribution is not fully verified. | Probe may observe and log metadata-only diagnostics; product prompt stays disabled until runtime validation passes. |
| `blocked_missing_bundle` | Target exists, but reliable macOS bundle ID is not known. | Cannot be matched from macOS mic attribution; package or local install verification required first. |
...
| --- | --- | --- | --- | --- |
| Zoom | `us.zoom.xos` | `prompt_enabled` | `runtime_verified` | Start `mic:us.zoom.xos` and target removal/end were captured locally on 2026-07-08. |
| Yandex Telemost | `ru.yandex.desktop.telemost` | `prompt_enabled` | `runtime_verified` | Start `mic:ru.yandex.desktop.telemost` and target removal/end were captured locally on 2026-07-08. |
...
| Slack calls/huddles | `com.tinyspeck.slackmacgap` | `diagnostic_only` | `confirmed` | Slack desktop identity is confirmed, but call/huddle mic attribution still needs validation. |
| Webex Meetings/App | `com.cisco.webexmeetingsapp`, `com.webex.meetingmanager` | `diagnostic_only` | `seed` | Multiple Webex app flavors exist; verify which bundle emits mic attribution in current Webex. |
...
| Kontur Talk / Толк | `kontur.talk` | `diagnostic_only` | `package_verified` | Official DMG verified: `Толк.app`, version `3.6.0`, team `VEWAJ43QEN`. Needs live mic attribution. |
| MTS Link / МТС Линк | `ru.weteams.desktop` | `diagnostic_only` | `package_verified` | Official DMG verified: `МТС Линк.app`, version `0.87.0`, team `ZZ645F8B29`. Needs live mic attribution. |
| TrueConf | `org.trueconf.client` | `diagnostic_only` | `package_verified` | Official DMG and local app verified. Idle launch did not emit mic attribution; needs live call/mic-test attribution. |
| VK Calls | `com.vk.calls.native.1` | `diagnostic_only` | `package_verified` | Official DMG verified: `VK Calls.app`, version `1.44.39190`, team `FD3X58MN39`. Needs live mic attribution. |
...
| VINTEO | `com.vinteo.desktop` | `diagnostic_only` | `package_verified` | Official ARM64 DMG verified: `VinteoDesktop.app`, version `4.27.0`, team `U47995Q86Q`. Needs live mic attribution. |
| eXpress | `ru.unlimitedtech.express.desktop` | `diagnostic_only` | `package_verified` | Official ARM64 DMG verified: `eXpress.app`, version `3.68.38`, team `NUMFWSGG8Z`. Needs live mic attribution and product-scope decision. |
| Pachca / Пачка | `com.todesktop.240607opwvcw853` | `diagnostic_only` | `package_verified` | Official ARM64 DMG verified. ToDesktop bundle ID is non-obvious; needs live mic attribution and product-scope decision. |
...
| Target | Reason |
| --- | --- |
| Google Chrome / Chromium browsers | Browser mic attribution is too generic; use browser metadata path. |
| Yandex Browser | Browser mic attribution is too generic; use browser metadata path. |
| Safari | Browser mic attribution is too generic; use browser metadata path. |
| Microsoft Edge | Browser mic attribution is too generic; use browser metadata path. |
| Firefox | Browser mic attribution is too generic; use browser metadata path. |
| Opera | Browser mic attribution is too generic; use browser metadata path. |
| Krisp or other audio processing layers | Not a meeting app target; ignore if it appears as non-allowlisted attribution. |
...
2. A real call or microphone-test run emits `mic:<bundle_id>`.
3. Leaving/quitting the call removes that target attribution.
