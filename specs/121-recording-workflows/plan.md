...
| Visible Consent And User Control | PASS for Feature 121 baseline; superseded for target-scoped detection by Feature 124 | Manual Start remains; Feature 124 restores the visible eight-second countdown, automatic expiry start, immediate `Записать сейчас`, `Пропустить`, and per-target opt-in while active/paused capture keeps textual state and one-action Stop in main/titlebar/menu-bar surfaces; plain Escape cannot stop capture. |
