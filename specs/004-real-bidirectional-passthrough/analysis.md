...
| A4 | High | `apps/macos/Scripts/validate-real-bidirectional-passthrough.sh` | Default-off/no-autostart validation now runs unconditionally in the main validation script. |
...
| Default app launch must not autostart app-side bridge or heartbeat | Yes | T068, T073, T074, T077, T078, T079 | Covered by `default-passthrough-disabled-check.sh` and runtime probe `--expect-default-safe`. |
