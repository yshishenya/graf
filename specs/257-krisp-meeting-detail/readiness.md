...
## Готовность дополнения выбора формата — PR #6897

FR-020–024, R53/R54, requirements checklist8/8 и analyze CRITICAL0/HIGH0/MEDIUM0 приняты. T012–T015 выполнены на продуктовом6611b19ec08ec7d27d422f9de2e5c054da779fe6:180 PASS адресного PostgreSQL-набора, отдельные Chromium/WebKit PASS, установленный GRAF Dev100/200% и13/13 smoke, current-SHA governance-fast34385318470 PASS. Полный список больше не обрезается плеером и не прокручивает документ при перемещении по форматам. История найденных ошибок и границы доказательств — [validation/format-picker.md](validation/format-picker.md).

Локальный frozen governance восстановлен без изменения lock/исходников расширения: закреплённый specify1.0.1 и удаление сгенерированного Python-кеша. Scoped canon четырёх задач PASS; глобальная проверка сохраняет чужой дефект#6852, который не включён в эту правку. Issues#6854–6857 подготовлены к закрытию через merge PR; umbrella#6794 не закрывается этим дополнением.

Окончательный документационный HEAD требует собственного GitHub gate и installed exact-SHA сверки; результаты прикрепляются к PR перед снятием draft. Release-full, merge, production и выпуск относятся к следующему релизному этапу и здесь не выполнялись. Исторические результаты исходного F257 выше не подменяют приёмку нового меню.
