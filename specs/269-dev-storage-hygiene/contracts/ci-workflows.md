...
3. Ключ изменяется при изменении `apps/macos/Package.resolved` или версии Swift; литерал версии в ключе синхронизирован с `swift-version` в workflow (обе проверки используют `6.0.3`). При смене версии инструмента ключ обязан измениться.
4. Кэш не содержит секретов и не влияет на подпись/нотаризацию (эти шаги не используют кэш).
