...
После проверок пользователь явно подтвердил коммит, push и ревью в
существующем PR #6560. Точный SHA, результат повторного ревью и новый
`governance-fast` фиксируются в описании и комментариях PR после отправки.
До успешного нового run PR gate остаётся открытым; старый run от
2026-09-04 не подтверждает этот diff.
