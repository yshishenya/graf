...
only the owner response body, fallback redirect, and browser row state.

---

## Phase 6: Follow-up - Keep Owner Delete On The List (Priority: P1)

**Goal**: After confirmation, the owner sees the selected recording disappear from the list without a persistent status banner or detailed deletion report.

**Independent Test**: A web delete request returns an empty success body; the browser removes accepted rows, keeps failed rows, and stays on the list; direct lifecycle/report endpoints still pass.

### Tests

- [X] T011 [US1] Extend `apps/server/tests/integration/test_cabinet_hx_delete_feedback.py` to require an empty successful HX body and explicitly reject success, cleanup, report, or report-link content.
...
- [X] T013 [US1] Keep `apps/server/src/twobrain_rec_server/cabinet/deletion_rendering.py` and the legacy fragment template empty so successful deletion has no owner-facing status block.
- [X] T014 [US1] Change `apps/server/src/twobrain_rec_server/cabinet/web_routes/deletion.py` to return an empty success body for asynchronous requests and redirect non-JavaScript form submits back to the current meeting list instead of the report.
...
- [X] T016 [US3] Record the owner-flow/report boundary and no-banner behavior in `specs/053-recording-selection-delete/contracts/cabinet-selection-delete-contract.md`, `specs/053-recording-selection-delete/quickstart.md`, and `CHANGELOG.md`.
...
- T013 and T014 share the empty-body contract and run sequentially.
- T015 depends on T013 because the browser no longer parses a feedback fragment.
