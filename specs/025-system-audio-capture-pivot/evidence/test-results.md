...
## 2026-06-09 Start Failure Cleanup Manifest Truth Review

- Timestamp: `2026-06-09T07:25:51Z`
- Commit before change: `74a5eea`
- Scope: failed Record startup cleanup after system-audio and/or local writer
  resources have already started.
- Finding:
  - Stop finalization already preserved system-audio stop failure truth, but
    the Record-start catch path still finalized the local writer before stopping
    system audio.
  - If system audio had started and local writer was active, then a later
    startup failure occurred, local cleanup could write a manifest without the
    system-audio stop/release failure reason.
  - That could make a partially started artifact look less severe than the real
    capture state.
- Fix:
  - The Record-start failure path now stops system audio first and passes the
    returned `failureReason` into local writer cleanup.
  - `finalizeLocalRecordingForFailure()` now accepts a forced failure reason,
    passes it to `LocalRecordingWriter.stopAsync(failureReason:)`, and logs the
    manifest failure reason in the cleanup event.
  - Stop failure cleanup now also passes the failure reason returned by
    `releaseForTermination()`.
  - `ContractValidation` now checks the app source invariant that start/stop
    failure cleanup preserves system-audio failure truth into local manifest
    finalization.
- Validation:
  - `swift build --package-path apps/macos` passed.
  - `swift run --package-path apps/macos ContractValidation` passed.
  ...
  - `git diff --check` passed.
- Acceptance impact:
  - This hardens #308 and #313 by ensuring startup cleanup artifacts cannot hide
...
## 2026-06-09 Permission Evidence Parser Self-Test Review

- Timestamp: `2026-06-09T08:32:25Z`
- Commit before change: `ecf0e51`
- Scope: Permission matrix parser behavior for the five required manual
  grant/deny/revoke scenarios.
- Finding:
  - Permission acceptance required the five canonical scenarios to have
    `Result=passed`, but there was no synthetic self-test proving duplicate
    `not-tested` or `failed` rows and unknown future permission states do not
    count as acceptance.
  - This made the permission matrix parser less reviewable than the CPU,
    artifact, and duration parsers.
- Fix:
  - Added `--self-test-permission-evidence` to
    `apps/macos/Scripts/validate-system-audio-capture-pivot.sh`.
  - The self-test creates a temporary markdown table with the five required
    passed scenarios plus duplicate not-tested/failed rows and an unknown
    future state.
  - The self-test requires exactly five accepted rows.
  - `apps/macos/Scripts/run-system-audio-controlled-manual-gate.sh --self-test`
    now invokes the permission evidence parser self-test.
- Validation:
  - `sh -n` passed for the updated validator and manual harness scripts.
  - `apps/macos/Scripts/validate-system-audio-capture-pivot.sh --self-test-permission-evidence`
    passed.
  - `apps/macos/Scripts/run-system-audio-controlled-manual-gate.sh --self-test`
    passed and now includes the permission evidence parser self-test.
  - `apps/macos/Scripts/validate-system-audio-capture-pivot.sh --self-test-cpu-evidence`
    passed.
  - `apps/macos/Scripts/validate-system-audio-capture-pivot.sh --self-test-artifact-metadata`
    passed.
  ...
  - `apps/macos/Scripts/run-system-audio-controlled-manual-gate.sh --preflight`
    passed with `wake_assertion=held`. Fresh safe-launch evidence showed idle
    `maxCoreaudiodCpuPercent=0.00`, `maxAppHelperCpuPercent=0.00`,
    `maxAppHelperRssMB=93.75`, quit app/helper process count `0`, and no
    thermal/performance warning.
  - Fresh app log tail showed launch, main-window presentation, parked driver
    diagnostics, disabled auto-start, visibility check, termination cleanup, and
    passthrough stop events without fresh crash/hang/error markers.
...
## 2026-06-09 Runtime Start Fail-Closed Review

- Timestamp: `2026-06-09T09:31:05Z`
- Scope: focused review of system-audio runtime start/stop cleanup, app launch,
  logs, CPU, and thermal behavior.
- Finding:
  - `SystemAudioCaptureService.startRuntime` handled timeout cleanup, and the
    ScreenCaptureKit runtime had its own start-catch cleanup, but the generic
    runtime contract did not call `stop()` when `runtime.start()` threw before
    the start was accepted.
  - A partially initialized runtime could therefore fail start without the
    service making the fail-closed stop call.
- Fix:
  - On immediate runtime start error, the service now completes the start as
    failed and calls `runtime.stop()` when that failure wins the start race.
  - Added a regression test proving a partially started runtime receives one
    stop call and the service remains not running.
- Validation:
  - `swift build --package-path apps/macos` passed.
  - `swift test --package-path apps/macos --filter SystemAudioCaptureServiceTests`
    completed successfully in the local SwiftPM/CLT environment and compiled
    the focused test bundle. Full XCTest execution remains unavailable locally
    because this machine uses Command Line Tools without `xctest`.
  - `swift run --package-path apps/macos ContractValidation` passed.
