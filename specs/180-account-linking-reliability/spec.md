...
2. **Given** свежие точные proofs и отсутствие blockers, **When** пользователь подтверждает, **Then** текущий профиль остаётся основным, способы входа и доступ переносится атомарно, оба исходных пространства сохраняются раздельно, а пользователь получает один понятный путь повторного входа.
...
- **FR-013**: Каждая сторона MUST иметь ровно один personal workspace с активным owner membership; source personal workspace MUST после merge стать отдельным доступным linked workspace, а отдельно принадлежащие corporate/неизвестные ownership shapes MUST блокировать merge до mutation.
...
- **FR-017**: Settings MUST использовать слова «профиль», «способ входа» и «пространство» и MUST не показывать внутренние слова preview, merge intent, survivor, ownership conflict, provider subject или RLS.
...
- **Workspace root**: Personal или linked пространство, тип и привилегии которого должны сохранить единственность после merge.
...
- Объединение workspace content в одно пространство или перенос billing/calendar credentials между пространствами.
