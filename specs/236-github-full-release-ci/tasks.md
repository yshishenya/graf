...
- [ ] T013 [US3] After analyze, sync `specs/236-github-full-release-ci/tasks.md` to child GitHub issues #6416–#6429; track umbrella #6415 separately as a feature-level issue. Before closeout reconcile every child issue, `tasks.md`, reviewer evidence, and `python3 scripts/validate-issue-closeout.py --issue-json <issue.json> --tasks specs/236-github-full-release-ci/tasks.md --expected-sha <candidate-SHA>` output, including the required Russian closure comment (Issue #6428).
- [ ] T014 [US3] Run `speckit-converge` and record the final exact SHA, issue-closeout manifest and reviewer-owned checklist evidence in `specs/236-github-full-release-ci/quickstart.md` and the PR description (Issue #6429).
