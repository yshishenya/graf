...
протоколом встречи. Модель получает полную закреплённую каноническую
расшифровку, возвращает тематическую структуру и ссылки на сегменты. Сервер
...
prompt fixtures, закрытая полнокорпусная проверка, HTML/Markdown parity,
...
**Release Gate**: владелец разрешил выпуск. Предварительный технический
PR/релиз содержит только снятие model lock и настройки Langfuse на действующем
outcome contract: без незавершённого protocol pipeline. После согласованного
переключения GRAF/root/LiteLLM выполняются сравнение моделей и весь пригодный
корпус. Основной PR/релиз протоколов возможен только после приёмки корпуса.
Каждый релиз проходит exact-SHA review/CI, release-full, CD dry-run,
production smoke и CalVer publication. Это устраняет цикл D239-I01; разрешение
не заменяет runtime evidence и не разрешает изменение остальных настроек шлюза.
...
**Performance Goals**: две стадии (создание и независимая проверка), каждая с
...
1. `outcomes/prompts.py` определяет один strict nested protocol schema и
   ...
   ссылок; решение дополнительно имеет ссылку на принятие, задача — раздельные
...
2. `cli/langfuse_prompts.py` компилирует подробный adaptive meeting-minutes
   ...
   Root bundle закрепляет также exact child `graf/meeting-outcome/verify`,
   который проверяет весь документ, финальные решения/обязательства и полноту
...
3. `outcomes/ai_service.py` сохраняет проверенный документ непосредственно в
   ...
   всех source refs. Existing Generation Call, prompt snapshot, stale/deletion
   fences и публикация не меняют смысл. Черновик и проверка имеют call_sequence
   1 и 2; повтор Temporal не вызывает модель заново. Проверка ссылается на hash
   точного черновика, transcript и root; publish требует завершённые обе стадии.
...
8. Закрытый corpus-run перечисляет все доступные встречи, исключает только
   доказуемо непригодные, создаёт новые протоколы, программно проверяет ссылки
   и позволяет Codex сопоставить каждый результат с полной расшифровкой.
   Повторяющаяся ошибка исправляется в общем prompt/validator/renderer, после
   чего весь пригодный корпус запускается заново.
...
5. Private corpus: 100% accessible meaningful meetings, full-transcript review
   against FR-029, safe aggregate counts and reasons only; full rerun after the
   last shared correction.
6. Closeout: focused tests throughout, `speckit-converge`,
   `infra/scripts/ci-local.sh --fast`, then `infra/scripts/ci-local.sh --full`
   on the frozen release candidate. Для каждого из двух выпусков применяются
   docs/agent-guidance/release-and-validation.md и отдельный exact-SHA gate.
...
переиспользовать его lifecycle и source controls. Единственный новый runtime
...
параллельный v2 renderer. Две стадии используют существующий durable call
...
semantic eligibility. Начальный вариант — полная расшифровка в обеих стадиях.
Если проверка выявит потери дальнего контекста даже внутри окна, необходимо
добавить детерминированные тематические части с полным покрытием и отдельными
call identities, обновить root и повторить весь корпус. Скрытое усечение и
исключение длинной пригодной встречи ради PASS запрещены.
