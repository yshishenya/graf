...
- [ ] T021 [US1] По скриншоту владельца от 2026-09-07 исправить профиль в `cabinet.js`/`cabinet.css`: hover открывает только одно подменю, переход в другое/уход закрывает старое, зазор не мешает перейти внутрь; keyboard/touch/Escape сохраняются. Убрать отдельные рамки вариантов темы, смягчить общий контур и радиусы. Состав/порядок прежний. Регрессия в существующем `CabinetSidebarRuntimeTests.swift`; штатный совместный Dev после передачи стенда от F249. Владелец #6754.
