...
## Исправление по реальному Dev, 2026-09-08

В установленном `dev-7b37ee8cf134` запрос был отклонён HTTP 403. Автосохранение отправляло multipart без CSRF-заголовка, тогда как сервер читает CSRF из тела только для urlencoded-форм; соседняя форма аккаунта уже использует X-CSRF-Token. Кроме того, выбранное значение theme пропадало, поскольку radio отключались до FormData. Обе ошибки устранены в существующем обработчике. Защита сервера и обычная форма не меняются.

Расширен существующий testProfileThemeAutosaveKeepsCurrentRoute: проверяет переданные theme/CSRF и отказ с возвратом последнего сохранённого выбора. До исправления 4 assertions FAIL, после — PASS. Server shell/settings checks: 148 PASS; node --check и git diff --check PASS. Нативный повтор после установки нового SHA фиксируется отдельно.
