...
## CUA runtime pass после разблокировки 2026-09-07

Установленный `GRAF Dev` доступен в окне 1040×680. На реальном native shell
проверены внешний скруглённый контур, свернутая/расширенная правая панель,
сохранение кнопки начала записи и матовая непрозрачная поверхность. Запись не
запускалась.

Чтобы увидеть кабинет без передачи реальных данных, на время проверки был
подменён только локальный Dev API тестовым сервером
`tests.fixtures.calendar_visual_ui_harness`; приложение продолжало работать как
`GRAF Dev`, bundle id `pro.2brain.graf.dev`. После проверки тестовый сервер
остановлен, штатный контейнер `graf-dev-api-1` возвращён.

В кабинете подтверждены исходная левая панель в свернутом и расширенном виде,
