...
T034: открыть «Меню GRAF» через app menu, выбрать «Выйти из GRAF»; процесс должен завершиться через cleanup_finished, а не timeout/kill. Повторить внешние клики и обновление состояния при открытом меню. sample не должен удерживать DispatchQueue.main во всём NSMenu event loop.
