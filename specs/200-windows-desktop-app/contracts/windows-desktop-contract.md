...
### Уточнение 2026-09-18: пауза по `Retry-After` (T083/R10)

- Ответ 429 с заголовком `Retry-After` — просьба подождать, а не отказ. Windows
  читает заголовок и не начинает новую попытку, пока пауза не истекла; обычное
  расписание в 30 секунд при этом отступает.
- Пауза не расходует лимит автоповторов: строка остаётся в состоянии `retry` с
  причиной `rate_limited`, счётчик попыток не растёт. Иначе ожидание, которого
  потребовал сервер, само приближало бы запись к исчерпанию бюджета.
- Нечитаемое или пустое значение трактуется как 60 секунд, как в macOS; пауза
  длиннее суток не удерживается — клиент снова спрашивает сервер сам, и сервер
  может попросить ещё раз. Пауза живёт в памяти планировщика: перезапуск
  приложения означает новый запрос, на который сервер ответит тем же 429.
- Пауза применяется только к 429. Сетевой сбой, 408 и 5xx идут обычным путём с
  собственным расписанием и расходуют попытку, как раньше. Форму HTTP-даты,
  которую понимает macOS, сервер GRAF не присылает: он всегда отправляет
  секунды, поэтому Windows ограничивается целыми секундами.

Регрессии: `DesktopUploadRecoveryTests.cpp` (`testRateLimitPause`) проверяет
разбор заголовка, отсутствие расхода попыток, запрет новой попытки внутри
паузы, её истечение и сохранение обычного пути для отказа без заголовка.

### Уточнение 2026-09-18: один снимок аккаунта на сессию (T083/R12)

- Требование «текущий пользователь и рабочее пространство подтверждаются
  `GET /api/v1/auth/me` с текущей сессией» относится к подтверждению аккаунта, а
  не к каждой попытке отправки. Оболочка подтверждает аккаунт для текущего токена
  сессии и передаёт этот снимок в полёт; транспорт использует снимок вместо
  нового `/auth/me`. Порядок «сначала `/desktop/settings/spaces`, потом `/auth/me`»
  сохраняется для первой проверки, когда пространство ещё неизвестно.
- Снимок не является полномочием сам по себе. Он принимается, только если
  описывает настоящий аккаунт (корректные UUID пользователя и пространства) и
  совпадает с рабочим пространством, с которым едет. Иначе отправка блокируется,
  как при отсутствии подтверждения.
- Смена токена сессии стирает снимок и отменяет запланированную отправку до того,
  как она начнётся, поэтому пара «токен → подтверждённый аккаунт» остаётся
  согласованной. Проверка совпадения владельца записи перед отправкой сохраняется.
- Если сессия отозвана на сервере, запросы отправки получают 401/403 и строка
  переходит в состояние «нужен вход»; отдельный опрос `/auth/me` для этого не
  требуется.

Регрессии: `DesktopUploadRecoveryTests.cpp` (`testConfirmedIdentitySnapshot`)
считает обращения к серверу и проверяет, что со снимком их нет, без снимка
клиент спрашивает сам, а несовпадающий или отменённый снимок не даёт полномочий.

### Уточнение 2026-09-18: удаление записи — ворота и область запроса (T083/R8)

- Удаление записи начинается с чтения `GET /api/v1/desktop/notification-context`.
  Ответ содержит пользователя, рабочее пространство и
  `recording_deletion_protocol_version`; мутация допускается только при версии
  `1`. Без этого клиент не отличит старый сервер, который молча игнорирует
  заголовки ожидаемого аккаунта, от нового, а нечитаемая квитанция пришла бы уже
  после удаления.
- Область запроса передаётся заголовками `X-Graf-Expected-Actor` и
  `X-Graf-Expected-Workspace`, как в macOS. Оба заголовка отправляются вместе, и
  запрос не уходит вовсе, если область не описывает настоящий аккаунт: сервер
  прочитал бы запрос без них как запрос без области.
- Цель адресуется одной проверенной частью пути: локальная запись — по правилам
  идентификатора, встреча — только как UUID. Тела запросов совпадают с
  контрактом сервера: `extra="forbid"` и литерал подтверждения
  `Delete this meeting everywhere GRAF controls.` для встречи, `operation_id`
  как UUID для отмены локального источника.
- `POST /api/v1/desktop/recordings/lifecycle` принимает от 1 до 100 целей.
  Разбор ответа строгий: непонятное состояние, неизвестный тип цели или
  некорректный идентификатор отвергают весь ответ, а не отдельную строку, потому
  что частичный список скрыл бы выбранную пользователем запись.
- Квитанция считается подтверждением только при известном `receipt_type`
  (`meeting_deletion` или `origin_cancellation`), фазе `accepted` и корректном
  идентификаторе запроса и цели. Незнакомая форма — не подтверждение, даже если
  пришёл код 202.
- `local-purge-tasks` (создание, список, подтверждение) и мост кабинета
  (`grafLocalRecording`, `deleteSelection`, `deletionCompleted`) в это уточнение
  не входят: версия моста не выставляется раньше настоящей серверной операции,
  иначе подтверждение удаления было бы фиктивным.

### Уточнение 2026-09-18: задачи очистки локальных копий (T083/R8, часть 2)

- Сервер создаёт задачу очистки, когда принял удаление встречи, и просит это
  устройство удалить локальные копии и ответить тем, что удалось доказать.
  Задача адресуется встречей (`POST /api/v1/desktop/meetings/{meeting_id}/local-purge-task`,
  без тела), читается списком (`GET /api/v1/desktop/local-purge-tasks`) и
  подтверждается по идентификатору задачи
  (`POST /api/v1/desktop/local-purge-tasks/{task_id}/ack`).
- Понимаются только те типы и состояния, которые сервер действительно может
  ...
  `local_expiry_relied_upon`. Незнакомое значение — не работа, за которую клиент
  может отвечать, поэтому такая задача отвергается целиком.
- Одна непригодная задача отвергает весь список: частичный список оставил бы
  устройство отвечающим за очистку, которой оно не видело. Список длиннее 200
  задач не принимается.
- Подтверждение несёт код доказательства из собственного словаря клиента
  (`local_artifacts_deleted`, `local_tombstone_verified`, `local_purge_failed`,
  `local_purge_unverified` и подобные) и необязательные поля клиента и времени
  завершения; необязательные поля опускаются, а не отправляются как `null` —
  так же кодирует их macOS-клиент. Подтверждение говорит, что удалось доказать, а
  не что запрос был отправлен.
- `ack_url` из ответа сервера остаётся в ответе, но **не используется как
  адрес**: Windows всегда строит путь из проверенного идентификатора задачи.
  Клиент, который пойдёт по адресу из ответа с приложенной сессией, позволил бы
  ответу решать, куда эта сессия попадёт. Это сознательное расхождение с macOS,
  который берёт из `ack_url` путь.
- Сопоставление задачи со строками очереди и сама очистка локальных копий с
  последующим подтверждением входят в R1: до появления пользовательского удаления
  такие задачи сервер не создаёт.

### Уточнение 2026-09-18: выполнение задач очистки (T083/R8, часть 3)

- Задача сопоставляется со строками очереди **только** по идентификатору встречи,
  который вернул сам сервер. Задача без идентификатора встречи не совпадает ни с
  чем, а не со всем.
- Удаляются только те локальные копии, которые сервер уже считает своими
  (`uploaded`). Копия, отправка которой не завершилась, не удаляется: это
  уничтожило бы незавершённую отправку, а не очистило бы буфер.
- Удаление идёт тем же безопасным путём, что и подтверждённое пользователем:
  проверка каталога внутри хранилища, отсутствие второго владельца пути, запись
  намерения в реестр до операции ОС, перенос в Корзину Windows. Отличается только
  доказательство: у задачи сервера своё (`serverRequestedPurge`), и оно не может
  быть получено из меню — пользователь эту копию не подтверждал.
- Тип задачи, который клиент не может проверить (`purge_local_exports`,
  `confirm_local_expiry`), не трогает файлы и честно отвечает
  `failed` / `local_purge_unverified`. Подтверждение говорит, что удалось
  доказать, а не что запрос пришёл.
- Доказанное удаление записывается в реестр (`purge_acknowledgements`) до отправки
  ответа. Поэтому потерянный ответ не превращает выполненную очистку в неудачу при
  повторном вопросе сервера. Записываются только доказанные удаления: неудача не
  замораживается, иначе локальную копию нельзя было бы удалить после того, как
  отправка завершится. Список ограничен тем же пределом, что и список задач
  сервера (200).
- Расхождение с macOS: macOS отвечает от терминальной строки, которую оставляет в
  очереди. Windows удаляет строку вместе с копией, поэтому хранит запись о
  доказанных ответах, а список ограничен. Поведение для пользователя то же: строка
  исчезает из кабинета, а повторный вопрос сервера получает тот же ответ.
- Встречи, по которым это устройство подтвердило удаление, но задачи ещё нет,
  запрашиваются у сервера адресно: сервер создаёт задачу со своей стороны, и
  адресный запрос — это и есть сопоставление.

Регрессии: `DesktopDeletionLedgerTests.cpp` — сопоставление по встрече, удаление
только завершённых копий, подтверждение доказанного удаления, честный отказ по
непроверяемому типу, запрос задачи по подтверждённому удалению, отказ на
нечитаемом реестре, сохранение доказательства между запусками и повторная попытка
после неудачи.

### Уточнение 2026-09-18: что кабинет видит об удалении (T083/R5)

- Вместе со строками кабинету передаются удаления в работе и признак скрытых
  записей: `window.GRAFLocalRecordings.update(rows, operations, recoveryRequired)`
  — ровно тот вызов, который делает macOS. Без него страница не могла напечатать
  ни состояние сохранённого запроса, ни уведомление о скрытых записях, потому что
  поля просто не приходили.
- Фаза удаления передаётся теми же именами, что хранит реестр (`queued`,
  `sending`, `resolving`, `accepted`, `verified`, `rejected`): одно написание
  означает, что страница и приложение не могут разойтись в том, в каком состоянии
  удаление. Причина ожидания переводится в словарь страницы (`rate_limit` →
  `rateLimit`, `server_update` → `serverUpdate`), а незнакомая передаётся как
  есть: страница тогда печатает фазу, а не состояние, которого ей не называли.
- Цель передаётся в той форме, которую читает страница (`target.meeting._0` или
  `target.ownOrigin._0`) — это кодирование перечисления, которое присылает macOS.
  Ссылка на отчёт об удалении появляется только тогда, когда встречу назвала
  квитанция: у локальной копии квитанция называет копию, а не встречу.
- Строка получила два признака, которые читает диалог удаления:
  `deletionIsLocalOnly` — серверу об этой записи не сообщали ни разу, поэтому
  удаление не тронет сервер; `localDeletionPending` — пользователь уже просил
  удалить копию, а файлы не подтверждены удалёнными. Оба признака выведены из тех
  же правил, по которым действует исполнитель удаления, поэтому обещание в
  диалоге и поведение не могут разойтись.
- `recoveryRequired` передаётся, но на Windows всегда `false`: macOS скрывает
  строки, доступ к жизненному циклу которых недоступен, и объясняет это
  уведомлением, а Windows показывает строку с её причиной («Нужен исходный аккаунт
  записи»). Ложное уведомление о скрытых записях было бы неправдой.
- Удаление, прерванное между записью намерения и операцией «Корзины», доводится до
  конца на потоке владельца в обычном проходе восстановления. Записанное намерение
  и есть подтверждение пользователя, поэтому отметка `local_delete_pending` не
  остаётся навсегда, а обещание «очистка ещё не завершена, повторим автоматически»
  становится правдой.

Регрессии: `DesktopDeletionLedgerTests.cpp` — обещание локального удаления только
для записи, о которой сервер не знает, и доведение прерванного удаления до конца;
`DesktopDeletionProtocolTests.cpp` — словарь причин ожидания.

Регрессии: `DesktopDeletionProtocolTests.cpp` — пути задач, разбор одной задачи и
списка со всеми типами и состояниями, отказ на непригодной задаче, точные тела
подтверждения и локальные отказы, которые не доходят до сети.

Регрессии: `DesktopDeletionProtocolTests.cpp` — ворота и версия протокола, сборка
путей, точные тела запросов, разбор жизненного цикла и квитанций, заголовки
области и локальные отказы, которые не доходят до сети.
