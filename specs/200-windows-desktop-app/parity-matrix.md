...
| Значок окна | Знак GRAF из ресурсов приложения | Знак GRAF из `Installer/Assets/Graf.ico` через `AppWindow::SetIcon` | Закрыто 2026-09-18: раньше в заголовке был системный значок-заглушка |
| Отказ одного источника | Дорожка помечается `degraded`, запись сохраняется | Отказ микрофона переводит сессию в `degraded` и запись продолжается; причина едет в `CaptureFinalization::degradedReason` | Микрофон закрыт 2026-09-18; поведение при отказе только системного звука ещё решается на реальном железе |
