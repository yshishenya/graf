...
## Постоянная настройка и проверенная версия приложения

- [X] CHK026 Разделены ли постоянный ключ настройки и точное доказательство доверия, включая подмену ключа, несколько версий, конфликт продуктов и сохранение Never при обновлении? [Consistency/Coverage, Spec §US5/AC7–9, Plan §Phase 5, Data Model §9]
- [X] CHK027 Обоснован ли переход дорелизного пустого каталога к V2 без удаления данных, определены ли default/bulk/ошибка записи/возвращение продукта и границы утверждения первой Teams-записи? [Compatibility/Completeness, Spec §US5/AC8–9, Plan §Phase 5, Quickstart §Постоянные ключи]

### Независимая проверка US5/AC7–9 — 2026-09-06

**PASS — CHK026/027, только качество требований.** `speckit-analyze`,
действующий высокорисковый срез T082; проверены три новых критерия, их связь
с продолжением T082 и сценариями quickstart. Замечаний в этой области нет.

- CHK026: Spec US5/AC7–9, Plan Phase 5 и Data Model §9 сохраняют все три
  режима по `targetKey`; точный `identityKey` остаётся у детектора, вопроса и
  слежения за записью. Членство дополнительно проверяет `targetKey`; подмена,
  конфликт продуктов и противоречивые названия отклоняются. Несколько версий
  дают одну строку продукта, предел реестра — 64 проверенные записи.
- CHK027: отдельный V2 в HKCU не читает и не меняет V1/global; пустой прежний
  рабочий реестр и запрет сохранения для неподтверждённой цели обосновывают
  отсутствие переноса. Отсутствие V1 в VM принято из сообщённой проверки,
  самостоятельно не перепроверялось. Определены Ask для нового приложения,
  общий выбор только для текущего каталога, сохранение правила при исчезновении
  и возвращении продукта, явный отказ записи без смены выбора и отрицательные
  сценарии повреждения V2. Сохранённый ключ не разрешает неподтверждённый EXE.
- `microsoft_teams_new` ограничен проверенной ARM64-версией подписанного пакета
  Microsoft; требуются точные хеши EXE/сертификата, происхождение в research.md
  и нативная проверка подписи при наблюдении. Нет автоматического добавления,
  утверждения поддержки всех версий/x64 или доказательства настоящей встречи.

Constitution 7 сохраняется: локальный выбор, технические условия, индикатор и
Stop без юридических/серверных разрешений. Проверка исходников ограничена
обоснованием прежнего пустого реестра и сохранения настроек, не приёмкой кода.
Метаданные первой подписи, реализация и испытания не одобрены; T082/T084 остаются
открытыми. Изменены только две новые отметки и эта запись; прежнее содержимое
сохранено. VM, сборки, тесты, GitHub и частные данные не использовались.
