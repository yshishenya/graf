...
- [X] T082 [P] [US5] Перенести локальную трёхрежимную автозапись Constitution 7 в `apps/windows/RecApp/MeetingDetection/`, `apps/windows/RecApp/Shell/AutomaticRecordingPrompt.*` и соответствующие `apps/windows/Tests/`; подключить подтверждённый Windows detector и prompt к shell после T081, сохранить countdown/remember/negative cases (FR-016, SC-007). В совместном с T081 срезе удалить отменённые `recordingPolicyAllowed`/`consentSatisfied`, мёртвые reason/copy в `Permissions/WindowsReadinessGate.*` и `Contracts/WindowsDesktopContracts.h`; до изменения кода скорректировать тесты технических отказов и проверить все позиционные инициализации. Правки AppMain выполнять последовательно с T081. Состояние на 2026-09-17: отменённые поля и мёртвые причины удалены, `targetKey` и `identityKey` разделены, автозапись подключена к оболочке (выбор режима, условия, отсчёт, `AutomaticRecordingPrompt::view`); в упакованном приложении проверен отрицательный случай — за 90 секунд без активного подтверждённого источника запись не началась, приложение устойчиво. Осталось: положительный случай настоящей встречи, полный каталог приложений и миграция предпочтений. Дополнено 2026-09-18: живая проверка каталога и настроек упирается в отсутствие входа в аккаунт на стенде; содержимое страницы не читается ни деревом доступности, ни отладочным портом. Снимок окна показывает, что кабинет рисуется и тёмное оформление применяется. Дополнено 2026-09-18 (раунд 26): окно «Настройки GRAF» проверено живьём — у «Для всех приложений» и у «Microsoft Teams» стоит «Спрашивать», рядом пояснение про 8 секунд и «Запомнить выбор». Само поведение (обнаружение встречи → вопрос → отсчёт → запоминание) требует живого обнаружения встречи и остаётся открытым. Дополнено 2026-09-18 (раунд 37): настройки трёх режимов проверены живьём — список «Всегда»/«Спрашивать»/«Никогда», отдельная настройка для Teams, выбор сохраняется между открытиями окна. Живое срабатывание упирается в намеренное свойство: определитель доверяет только подписанному процессу (отпечатки файла и издателя), поэтому нужен настоящий Teams или Zoom в машине. Доказательства: `specs/200-windows-desktop-app/validation-2026-09-17.md`, раздел раунда 37. Дополнено 2026-09-18 (раунд 41): в машину поставлен Яндекс Телемост (2.42.4.9707, подпись YANDEX LLC), его точные хеши внесены второй записью каталога с ключом `yandex_telemost` из общего продуктового каталога; настройки показывают новую строку. Тогда же найдена настоящая причина паузы автозаписи: согласие на микрофон для упакованного приложения стояло в положении `Prompt`; после выдачи согласия панель пишет «Ожидаем встречу». Живого срабатывания нет: Teams требует входа, а окно Телемоста в этой машине не рисуется (x64-приложение под ARM64 через эмуляцию). Подробности в `validation-2026-09-17.md`, раздел раунда 41. Продолжение того же раунда: живая встреча в машине получена — приложение встречи создало комнату и держало активный микрофон (журнал: 48 кГц, `ConferenceSession`, качество `EXCELLENT`), но участник был один, поэтому потока воспроизведения не было и GRAF приглашение **не** показал. Это правильное поведение: определитель требует одновременно захвата и воспроизведения у проверенного процесса, и микрофон сам по себе встречей не считается. Для положительного случая в комнате должен говорить второй участник (второе устройство или веб-клиент с микрофоном; последнее отправило бы звук из комнаты на сторонний сервис, поэтому без разрешения владельца не выполняется). Дополнено 2026-09-19 (раунд 50): **положительный случай получен живьём**. В машине развёрнуто окно приложения Yandex Telemost, приложение вошло в комнату и держало оба потока; GRAF обнаружил встречу сам — в дереве доступности его окна появилось уведомление с обратным отсчётом («через 8 секунд», счётчик `0:06`) и кнопка «Открыть приложение GRAF». Запись началась **без нажатия** «Начать запись»: в режиме «Спрашивать» она стартует сама через 8 секунд, как и описано в настройках. В `%LOCALAPPDATA%\GRAF\recordings\` появился пакет `0ABCBDA5-9312-4DE6-B3A8-6B68F156EA39`: `schema_version=local-recording-manifest.v5`, `capture_status=degraded`, `capture_reason_code=clock_discontinuity`, `duration_ms=6430`, `trusted_prefix_frames=643`, медиа 205 804 байта `sha256=70b938f0…`, воспроизведение 79 262 байта `sha256=19624984…`. Каталог проверенных приложений закрыт двумя подписанными записями (`microsoft_teams_new`, `yandex_telemost`) — теми же ключами, что и в общем продуктовом каталоге; миграция предпочтений не нужна, потому что на Windows это первая версия и мигрировать нечего. Машинная граница (`degraded` / `clock_discontinuity`) записана в раунде 45 и относится к захвату, а не к срабатыванию. Согласие на микрофон было переведено в `Deny` до начала записи, тестовый пакет удалён после записи фактов, файлы владельца не трогались. Подробности: `validation-2026-09-17.md`, раздел раунда 50.
...
- [ ] T087 [US2] Перенести жизненный цикл удаления записи F262 в Windows: удаление из кабинета, из очереди и из локального хранилища как одна операция с повторяемым результатом, без осиротевших каталогов и без воскрешения удалённой записи при восстановлении очереди. Файлы: `apps/windows/RecApp/Upload/DesktopLocalPurgeService.*`, `Upload/DesktopUploadQueueService.*`, `Web/WebViewRoutePolicy.*`, `AppMain.cpp`, тесты удаления. Дополнено 2026-09-19 (раунд 42): живые инварианты очереди и хранилища проверены — 12 записей очереди против 12 каталогов, совпадение один к одному, осиротевших каталогов 0, реестр удалений (`deletion_operations`, `purge_acknowledgements`) есть и пуст, у всех записей `attempts=0`. Не проверено живьём: само удаление из кабинета до конца — реестр пуст, потому что ни одна запись ещё не удалялась. Доказательства — в `validation-2026-09-17.md`, раздел раунда 42. Дополнено 2026-09-19 (раунд 43): найдена и исправлена причина, по которой удаление из кабинета не доходило до приложения — команды `native_settings` и `delete_selection` отсутствовали в списке разрешённых команд моста (`WebViewBridge::isAllowedWebCommand`), поэтому отбрасывались с `validation-rejected`. После правки журнал моста показывает `accepted native_settings`; сам путь «строка кабинета → задача очистки → локальное хранилище» живьём ещё не пройден из-за отсутствия кнопки удаления в строке. Доказательства — в `validation-2026-09-17.md`, раздел раунда 43. Дополнено 2026-09-19 (раунд 44): живьём открыт отчёт об удалении в кабинете Windows — жизненный цикл `active purge complete`, состояния файлов под контролем GRAF (два `purged`, три `purge requested`); найдены действия строки локальной записи (отметка выбора и красная корзина появляются после взаимодействия со строкой). Локальное подтверждение очистки всё ещё пусто: короткая ручная запись пакета не оставила, и это правильно — сработало правило T086 (F6796) о пороге короткой записи при нормальной остановке; для проверки удаления нужна запись длиннее порога. Удалять записи владельца ради проверки не стал. Доказательства — `validation-2026-09-17.md`, раздел раунда 44. Дополнено 2026-09-19 (раунд 45): новая локальная запись в этой машине сейчас не создаётся — приложение честно отвечает «Не удалось сохранить запись: системный звук недоступен» и ничего не сохраняет. При этом среда в принципе работает: два более ранних пакета лежат с `capture_status="normal"` и длительностью 9880 и 9600 мс. Объяснение «устройство простаивало» отброшено: заход на 45 секунд без постороннего звука дал тот же отказ. Точную причину можно назвать на физическом x64 — отложенный гейт T071. Путь удаления на своих данных поэтому не пройден; команда удаления проверку моста уже проходит, отчёт об удалении открывается.
- [ ] T088 [US2] Перенести сохранение сессии приложения и аккаунта F6795/F6794 в Windows: сессия и выбранный аккаунт переживают перезапуск, выход очищает ровно своё, повторный вход не создаёт вторую сессию. Файлы: `apps/windows/RecApp/AppMain.cpp`, `Upload/DesktopApiClient.*`, `Storage/AtomicFileStore.*`, тесты. Дополнено 2026-09-19 (раунд 42): сессия и аккаунт проверены живьём — после переустановки пакета и перезапуска кабинет открылся со встречами аккаунта без повторного входа. Доказательства — в `validation-2026-09-17.md`, раздел раунда 42.
- [ ] T089 [P] [US2] Перенести управление уведомлениями F249 в Windows: пользователь может выключить уведомления, выключенное состояние переживает перезапуск и не ломает запись. Файлы: `apps/windows/RecApp/Shell/`, `AppMain.cpp`, настройки, тесты. Дополнено 2026-09-19 (раунд 42): живьём переключатель уведомлений в интерфейсе машины не найден, подтверждение остаётся на модульных тестах — это записано как граница в `parity-matrix.md`. Дополнено 2026-09-19 (раунд 48): исправлено в оболочке Windows — общая серверная страница больше не называет Windows «Маком»: слова приводятся к платформе, кнопка системных настроек скрыта (у этой страницы на Windows нет системного разрешения, которое она могла бы открыть). Серверная копия не трогалась, поэтому macOS не меняется. Правка покрыта проверкой `testTheSharedSettingsPageStopsClaimingToBeAMac`; полосы сборки, пакета и установки прошли. Граница: исправленный текст глазами на живой машине не перепроверен — боковой список разделов настроек не отзывается на щелчки по координатам (пять попыток, три набора координат), при этом щелчки по содержимому той же страницы работают.
...
- [ ] T090 [P] [US1] Довести настройки, токены оформления и пользовательское время до текущего macOS: единый список выбора F6793, упрощение интерфейса F255, токены F240 и пользовательское время F252. Входит в срез T081 по `apps/windows/RecApp/AppMain.cpp`; сначала расширить тесты, затем удалить статические заглушки. Дополнено 2026-09-19 (раунд 42): живьём видны три режима автозаписи («Всегда», «Спрашивать», «Никогда»), отдельные строки приложений и местное время в кабинете; мост оформления подтверждён строками журналов `accepted app_appearance` и `received #F5F6F8 announced=system dark=0`. Доказательства — в `validation-2026-09-17.md`, раздел раунда 42. Дополнено 2026-09-19 (раунд 47): три темы проверены живьём вместе с нативной панелью и заголовком окна; «Системная» следует теме машины.
- [ ] T091 [P] [US3] Перенести мост настроек встроенного кабинета F264 в Windows и сверить совместимость моста загрузки, авторизации и кабинета с master, записав matrix в `specs/200-windows-desktop-app/parity-matrix.md`. Не менять серверную бизнес-логику. Расширяет T083. Дополнено 2026-09-19 (раунд 42): мост настроек кабинета проверен живьём по журналам (`bridge.log`, `appearance.log`); matrix обновлена в `parity-matrix.md`, раздел живых проверок раунда 42. Дополнено 2026-09-19 (раунд 43): живая проверка вскрыла мёртвую ветку моста — `native_settings` и `delete_selection` не были разрешены в конверте, поэтому страницы настроек кабинета не работали вовсе. Исправлено, проверки расширены, журнал моста подтверждает `accepted native_settings`. Осталось: маковский текст страницы уведомлений на Windows (общая серверная копия) — нужно решение владельца. Дополнено 2026-09-19 (раунд 47): живая смена темы проверена целиком — «Светлая», «Тёмная» и «Системная» меняют и кабинет, и нативную панель, то есть объявление оформления действительно доходит из страницы до оболочки. Там же подтверждён путь восстановления WebView2: после отказа процесса обозревателя кабинет показал честное сообщение «Не удалось открыть кабинет … browser-process · 0x80004005» и кнопку «Повторить загрузку», которая восстановила кабинет без перезапуска приложения.
