...
- [ ] T085 [P] Довести оформление нативных окон до конца: заголовки окон уже в палитре приложения, но окно «Настройки GRAF» может остаться в прежней теме, если было открыто до смены оформления, а живое переключение темы из подменю кабинета в раунде 33 не дошло до приложения ни разу (в журнале моста нет новых `app_appearance`). Проверить, не теряется ли нажатие в подменю, открывающемся наведением, и при необходимости пересоздавать окно настроек при смене темы. Доказательства: `specs/200-windows-desktop-app/validation-2026-09-17.md`, раздел раунда 33.
