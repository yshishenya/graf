...
- [ ] T089 [P] [US2] Перенести управление уведомлениями F249 в Windows: пользователь может выключить уведомления, выключенное состояние переживает перезапуск и не ломает запись. Файлы: `apps/windows/RecApp/Shell/`, `AppMain.cpp`, настройки, тесты. Дополнено 2026-09-19 (раунд 42): живьём переключатель уведомлений в интерфейсе машины не найден, подтверждение остаётся на модульных тестах — это записано как граница в `parity-matrix.md`.
