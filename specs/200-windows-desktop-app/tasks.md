...
- [ ] T091 [P] [US3] Перенести мост настроек встроенного кабинета F264 в Windows и сверить совместимость моста загрузки, авторизации и кабинета с master, записав matrix в `specs/200-windows-desktop-app/parity-matrix.md`. Не менять серверную бизнес-логику. Расширяет T083. Дополнено 2026-09-19 (раунд 42): мост настроек кабинета проверен живьём по журналам (`bridge.log`, `appearance.log`); matrix обновлена в `parity-matrix.md`, раздел живых проверок раунда 42. Дополнено 2026-09-19 (раунд 43): живая проверка вскрыла мёртвую ветку моста — `native_settings` и `delete_selection` не были разрешены в конверте, поэтому страницы настроек кабинета не работали вовсе. Исправлено, проверки расширены, журнал моста подтверждает `accepted native_settings`. Осталось: маковский текст страницы уведомлений на Windows (общая серверная копия) — нужно решение владельца.
