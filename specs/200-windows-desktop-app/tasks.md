...
- [ ] T084 [P] Починить живое переключение оформления: смена темы внутри кабинета меняет страницу, но не оболочку — панель записи, рельс, панель инструментов и заголовок окна остаются прежними, пока не нажата перезагрузка. Мост `app_appearance` объявлен и в macOS, и в Windows; нужно найти, почему при живом изменении `data-theme` объявление не доходит, и закрыть расхождение. Доказательства: `specs/200-windows-desktop-app/validation-2026-09-17.md`, раздел раунда 30.
...
- [ ] T093 [P] [US2] Закрыть зависимость распространяемого пакета Visual C++ в MSIX: приложение собрано с `/MD`, а подавление управляемого импорта убрало из манифеста и `Microsoft.VCLibs.140.00.UWPDesktop`. Либо вернуть корректную зависимость и класть соответствующий `.msix` в `Dependencies`, либо перейти на статическую компоновку CRT. Признак закрытия: установка и запуск на образе Windows без распространяемого пакета Visual C++.
