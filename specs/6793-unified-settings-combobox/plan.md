...
- Dependencies: без новых зависимостей; DOM и AppKit NSTextField/NSPopover.
...
## Complexity Tracking
Новые библиотеки и отдельный UI framework не нужны. Штатный datalist не гарантирует единообразное открытие всех вариантов и запрет свободного значения; DOM helper нужен для этого контракта. На macOS использовать NSTextField и NSPopover с явным подтверждением щелчком/Enter. Проверка установленного NSComboBox выявила отсутствие надёжной границы между навигацией и подтверждением; AXShowMenu открывает контекстное меню редактирования. Публичные AppKit элементы сохраняют единое поле и отделяют запрос, активный вариант и сохранённое значение.
