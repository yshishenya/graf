...
- [X] T013 Исправить возвращение ручки resize при изменении размера свёрнутой панели и сохранить компактную цветную разметку спикеров по повторно проверенному Krisp в rendering.py и static/cabinet/cabinet.js, cabinet.css; дополнить specs/256-krisp-playback-panel/evidence/playback-panel-runtime-check.cjs проверкой цветной общей шкалы и измерением закрытого shell до/после resize и100/200%, перепроверить фактический рисунок обоих состояний в установленном GRAF Dev и обновить PR evidence согласно FR-009/FR-017, SC-002/SC-006 (partial). Все пути кода относительно apps/server/src/twobrain_rec_server/cabinet/. (Issue #6830)

## Phase 2: Convergence по обязательному review PR

- [X] T014 Пакетно загружать обсуждения и разрешённые имена внешних авторов в apps/server/src/twobrain_rec_server/cabinet/comments.py; проверить ограниченное число SQL и PostgreSQL RLS в apps/server/tests/integration/test_meeting_comments_read_review.py согласно FR-010/011 (partial). (Issue #6832)
- [X] T015 Исправить аудит changes permissions, организацию доступной owner-workspace проекции редактора и прямую ссылку на ответ после первой страницы в apps/server/src/twobrain_rec_server/api/cabinet.py; добавить apps/server/tests/integration/test_meeting_comments_api_review.py и прокрутку/фокус прямого ответа в static/cabinet/playback-comments.js с проверкой tests/browser/playback-comments.test.cjs согласно FR-010/011/016 (partial). (Issue #6833)
- [X] T016 Доказать существующий proof-bound account_merge доступ для comments/mentions/reactions; добавить после0090 миграцию ограниченных проекций имён и адресатов в apps/server/src/twobrain_rec_server/db/migrations/versions/ и регрессию apps/server/tests/integration/test_meeting_comments_merge_review.py согласно FR-015/016 (partial). (Issue #6834)
