...
### T007: exact-SHA PR gate

Коммит исправления `5e3fc608d7fe952c221d6e1564661cf148abc127`:

- governance-fast: PASS, https://github.com/yshishenya/graf/actions/runs/34711815328.
- pr-metadata: PASS, https://github.com/yshishenya/graf/actions/runs/34711815320.
- Оба обсуждения автоматического ревью разрешены после независимого разбора и регрессий. GitHub: OPEN, non-draft, MERGEABLE/CLEAN.
- CodeRabbit для нового коммита сообщил manual review required; его зелёный статус не считается новым содержательным review. Содержательная проверка выполнена независимым final_review.

Converge: T007 закрывает выявленный пробел FR-004; 7 FR, 3 SC и существующие acceptance scenarios сопоставлены с исходной приёмкой и дополнительными управляемыми проверками. Обязательных работ по реализации не осталось. Этот документационный коммит не меняет продуктовый код; готовность к Merge требует повторного успешного exact-SHA GitHub gate, который фиксируется в PR. Issues #6930–#6936 и #6955 сохраняют внешнее состояние OPEN до корректного merge/release closeout.

Пределы остаются явными: T006 проверена в установленном приложении до T007; новое согласование навигации проверено в настоящих WK/Coordinator автоматических тестах, без новой установки. Исправление стенда MinIO и повторная установленная проверка не включались в дополнительный PR-коммит. Это подготовка исходного кода к Merge, не выпуск или утверждение об обновлении установленного продукта.


## Релизная совместимость Swift 6.0.3 — v2026.09.13.1

Первый release-full `34714757745` для SHA `dea5fd6a467149f70a969f7737773dc22198aabc` остановил macOS-компонент при компиляции новых тестовых делегатов: старый SDK WebKit объявляет WKNavigationDelegate без MainActor. Два класса RenewalNavigationDelegate/RenewalActionCapture получили `@preconcurrency` на соответствии протоколу, как существующий TimeContextNavigationDelegate. MainActor и проверки выполнения сохранены; продуктовый код не менялся.

`swift test --package-path apps/macos --filter DesktopCabinetSessionBridgeTests`: PASS, 13 tests, 0 failures; `git diff --check`: PASS. Для точного закреплённого Swift требуется отдельный GitHub macos-diagnostic, а для нового итогового кандидата — новый полный release-full. Первый кандидат не публиковался и не развёртывался. Lane: active Spec Kit T007, тестовая совместимость при release integration.


Дополнительный macos-diagnostic `34715106256` подтвердил исправление делегатов, затем выявил nonisolated async imports WKHTTPCookieStore в старом SDK. Тесты используют три локальных MainActor-обёртки штатных callback API через checked continuation; store не передаётся на другой executor, предупреждения глобально не отключаются. Все прежние сценарии и ожидания сохранены. Локальный повтор: 13 PASS, 0 failures. Продуктовый код остаётся неизменным; требуется новый диагностический run точного SHA.


Повтор `34715405020`: Swift-компиляция прошла; выполнение остановилось в первом сценарии навигации с nil в implicitly unwrapped Optional. Единственный async evaluateJavaScript в новых тестах возвращал undefined после submit/click, несовместимый с прежним импортом результата Any. Добавлен явный результат true после действия; сама отправка/нажатие не меняется. Локальный повтор: 13 PASS, 0 failures; далее повторная проверка закреплённого SDK.
