...
- [ ] CHK001 Are local package eligibility requirements complete for `mic.wav`,
...
- [ ] CHK002 Are requirements defined for recordings that are saved locally but
  blocked from upload by quality/privacy gates? [Completeness, Spec US1/US5]
- [ ] CHK003 Are requirements explicit that `042` does not alter live capture,
...
- [ ] CHK004 Is "accepted local recording package" defined through measurable
  manifest/status fields rather than subjective audio quality language?
  [Clarity, Data Model]
- [ ] CHK005 Are track role mappings clear from local roles to upload roles and
  MediaScribe dual-track roles? [Clarity, Plan Source Code, Data Model]
- [ ] CHK006 Are derived or mixed tracks explicitly excluded from `042` upload
  eligibility unless a later spec changes that rule? [Clarity, Spec Out of
  Scope, Data Model]

## Requirement Consistency

- [ ] CHK007 Do upload requirements preserve existing leakage finalization and
  fail-closed transcription readiness gates? [Consistency, Current Baseline,
  Constitution I]
- [ ] CHK008 Do local retention requirements preserve local artifacts after
  upload until policy/deletion truth allows purge? [Consistency, Spec US6]
- [ ] CHK009 Are future AEC/speakerphone backlog items `037`-`041` kept separate
  from `042` runtime requirements? [Consistency, `docs/audio-capture-backlog.md`]

## Edge Case Coverage

- [ ] CHK010 Are permission revoked, protected audio, silent input, no frames,
...
- [ ] CHK011 Are local file missing and checksum changed cases addressed without
