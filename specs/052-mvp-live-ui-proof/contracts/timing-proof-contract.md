...
- metadata-only note explaining which intervals were measured.

## Target Rule

- A direct representative one-hour or near-one-hour production run may pass or
  fail the target.
- A short production run may prove pipeline health, but must not be
  extrapolated into a timing pass.
