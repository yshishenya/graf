...
def _health() -> dict[str, Any]:
    ...
    return {
        ...
        "ready": {
            ...
            "status": ready.get("payload", {}).get("status"),
        },
    }
...
def _owner_review() -> dict[str, Any]:
    ...
    outcome_states = {category: (truth.get(category) or {}).get("state", "missing") for category in OUTCOME_CATEGORIES}
    outcome_item_counts = {category: len((truth.get(category) or {}).get("items") or []) for category in OUTCOME_CATEGORIES}
