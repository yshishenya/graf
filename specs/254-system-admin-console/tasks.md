...
## US7
  ...
  - Исправлен actual-role дефект внутренних приглашений: ограниченная meeting-bound проверка членства, поиск и положительный HTTP-сценарий; прямое чтение чужих membership по-прежнему закрыто. Архивирование проверяет разрешение при новом приёме; уже принятый выбор сохраняется при повторе обработки. См. implementation-evidence.md. AI consumers подключены и проверены, включая автоматическую генерацию, сохранение принятых запросов и браузерный отказ. Остальные окна и lineage ещё открыты.
