...
### Допуск генерации по коммерческим правам

Новая генерация через summary-candidates, summaries/{type}/ensure, summaries/{type}/refresh и автоматические import/reconciliation проверяет общий resolver в транзакции Workspace → Meeting. Текущий единый вызов модели создаёт резюме и структурированные итоги вместе: требуются одновременно `ai_summary=true` и `ai_outcomes=true`. Запрет любого из них даёт 403 `commercial_ai_generation_denied`; недоступные условия — 503 `billing_access_unavailable`, без внутренних деталей. Это ограничение создания: сохранённые результаты, их просмотр и обработка уже принятых заданий не отзываются тарифом. Точный повтор принятого запроса возвращает прежний candidate; явный новый refresh требует действующих прав. Остальные ACL/источник/удаление остаются обязательными.

Автоматический отказ не создаёт slot, candidate или dispatch и не меняет выбранный формат. При импорте расшифровка остаётся processed; ожидаемый результат получает состояние blocked с безопасной причиной, вместо ложного generating. Позднее восстановление права допускает новую автоматическую генерацию через существующий reconciler. Независимое создание только резюме или только структурированных итогов потребует отдельного поддерживаемого генератора; этот выпуск не разделяет содержимое одного вызова модели.
