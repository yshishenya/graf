...
- Диапазон релиза выводится из опубликованного stable product release на строгой цепочке предков source. Уже опубликованный текущий source пропускается как база: последующая аттестация сохраняет проверки всех вошедших PR. Каждый commit принадлежит единственному merged PR, кроме ровно одного notes-only follow-up commit от release operator: он должен идти после последнего опубликованного CalVer, сохранять заголовок и feature marker, иметь уникальные структурные поля архивного `F<id>.yaml`, валидные фрагменты для всех отмеченных features и проверку добавленных строк на секреты, включая известные token prefixes. Linear rebase покрывается целиком, набор train сверяется точно; metadata-only commit не добавляет PR в train. Некорректная политика, отсутствующий предшествующий release/PR, неоднозначность, второй metadata-only commit, изменение кода или stale/failed proof блокируют действие.
