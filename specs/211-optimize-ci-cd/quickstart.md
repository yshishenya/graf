...
- Диапазон релиза выводится из опубликованного stable product release на строгой цепочке предков source. Уже опубликованный текущий source пропускается как база: последующая аттестация сохраняет проверки всех вошедших PR. Каждый commit принадлежит единственному merged PR, кроме узкого продолжения release-prep от release operator: один notes-only commit с неизменными заголовком/feature marker, структурными полями архивного `F<id>.yaml`, текущей CalVer и проверкой добавленных строк на секреты. Linear rebase покрывается целиком, набор train сверяется точно; metadata-only commit не добавляет PR в train. Некорректная политика, отсутствующий предшествующий release/PR, неоднозначность, изменение кода или stale/failed proof блокируют действие.
