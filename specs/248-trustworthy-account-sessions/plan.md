...
7. `settings_account_content.html`: единый список «Устройства и сеансы», кнопки только для действующих чужих сеансов; отдельные раскрываемые подробности и история; область рабочего пространства и фоновая активность объяснены. Существующие device endpoints сохраняются и проверяются, сырой дублирующий список registrations убирается.
