...
`получатель → безопасное письмо/CTA → подтверждение именно его адреса →
summary-only просмотр → ненавязчивый «Попробовать GRAF» → явный login/signup`.

CTA не блокирует просмотр, не создаёт аккаунт и не добавляет человека в
workspace автоматически. Attribution — только непрозрачный bounded id,
...
- Read.ai показывает путь от shared report к signup, но аккаунт появляется
  только после действия получателя. Это хороший onboarding pattern.
...
- Невидимо создать аккаунт по email: отвергнуто — требуется явное согласие.
