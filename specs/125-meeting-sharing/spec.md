...
доступно и как открыть запись. Если у него ещё нет GRAF, он может создать свой
...
2. **Given** получатель не вошёл в GRAF, **When** он нажимает основную кнопку,
   **Then** magic link подтверждает приглашённый email; если аккаунта ещё нет,
   GRAF создаёт personal account автоматически и сразу открывает разрешённую
   страницу записи. Получатель может просмотреть саммари и транскрипт с
   таймкодами, послушать запись, скачать аудио и сохранить combined export
   «расшифровка + саммари»; отдельные пароль, код и `/sign-up` не требуются.
...
- **FR-020**: Recipient surface MUST показывать разрешённый content после
  ...
  Для summary-only используется `Открыть итоги`, для full package —
  `Открыть запись`; явное действие может создать только personal account для
  приглашённого адреса.
