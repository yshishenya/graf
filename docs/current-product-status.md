...
## В разработке (2026-09-06) — Feature 239: полноценные протоколы встреч

- В ветке фичи реализован единый вложенный протокол: ключевые итоги, цели,
  темы, решения, задачи и источники с таймкодами. Формулировки остаются
  модельными; ГРАФ проверяет документ и ссылки, но не переписывает слова.
  Создание и независимая модельная проверка используют отдельные сохранённые
  вызовы с точными настройками из Langfuse.
- Это не описание выпущенного поведения. Технический PR #6661 снял
  ограничение одной моделью в коде; он не выпустил полноценные протоколы.
  Основная фича и приёмка качества ещё не завершены. Специальный ограничитель
  шлюза удалён по отдельному разрешению владельца; разные модели доступны.
  Для Gemini исправлена несовместимость отправляемой схемы без снятия
  серверных проверок. Пробные реальные результаты ещё имеют содержательные
  ошибки; готовность по синтетическим тестам не объявляется.
- Задача ограничена проверенным PR без merge, release, deploy и изменения
  production labels. Исключение для LiteLLM касалось только уже удалённого
  специального pre-call ограничителя. Общая активация с другими фичами принадлежит
  отдельной задаче владельца. Текущее частичное evidence:
  [проверка интеграции](../specs/239-full-meeting-minutes/validation/protocol-integration-review.md).
