...
- Отдельной регистрации как обязательного шага больше нет: первый вход по коду
  из письма или через внешнего провайдера создаёт аккаунт. Отдельная страница
  регистрации сохранена и продолжает работать, потому что на неё ведут
  реферальные ссылки. Ограничение частоты, срок жизни кода, привязка к браузеру
  и аудит входа не изменились.
- Каталог цен создаётся миграцией `0093_billing_catalog_seed` при обычном
  `alembic upgrade head` и не требует ручных действий оператора. Миграция
  идемпотентна и не перезаписывает уже настроенные значения; литерал редакции
  оферты связан тестом с публичной константой.
- Потеря ответа платёжного провайдера больше не оставляет деньги без доступа:
  платёж связывается с операцией по идентификатору из метаданных, когда поиск
  по сохранённому идентификатору платежа не дал результата. Выдача доступа
  остаётся однократной.
- Зависшая операция больше не блокирует новую оплату бессрочно: операция без
  сохранённого платежа с истёкшим ключом идемпотентности отменяется вместе со
  своим счётом, а перевод в неизвестное состояние по истечении 30 минут теперь
  применяется только к операциям с сохранённым платежом, поэтому кнопка
  «Продолжить платёж» живёт весь срок ключа.
- Продление делает три попытки списания заранее — за 72, 48 и 24 часа до даты
  окончания периода — и сообщает письмом о неудаче и о дате отключения доступа.
  Отказ карты больше не отключает доступ сразу: автопродление отзывается и
  подписка переходит на бесплатный тариф только при исчерпании попыток или по
  достижении даты окончания.
- Активная подписка больше не запрещает начать новую оплату, а новый период
  считается от текущей даты окончания. Поэтому человек может заплатить
  досрочно и перейти с месяца на год, не теряя остаток оплаченного периода.
  Повторное нажатие «оплатить» по-прежнему не создаёт второй платёж.
- Механизм аварийной остановки биллинга удалён полностью: из кода, настроек,
  инфраструктуры, тестов и документации. Штатный выключатель оформления оплаты
  сохранён, и проверка готовности ведёт себя как прежде.
- Установщик защиты вебхука подтверждает, что секрет на границе совпадает с
  секретом приложения: он отправляет записанный секрет напрямую в backend и
  считает провалом ответ `401` на обоих путях окружения.
- Реальные платежи ещё не включены: production указывает на тестовый магазин
  `1436758`. Переключение на боевой магазин `1430118` и проверка живой оплатой
  остаются операционными шагами по `docs/runbooks/billing-launch.md`.
