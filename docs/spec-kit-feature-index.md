...
| [127](../specs/127-settings-ia/spec.md) | Реализована; post-release auth handoff и полный settings inventory проверены локально | Защищённые HTML-поверхности без browser session переводят на `/login` с безопасным `next`; API сохраняют problem JSON, а «Уведомления» доступны из общей навигации. Production rollout этого follow-up ещё не выполнялся. |
...
| [148](../specs/148-local-workflows/spec.md) | Реализована: локальные CI и release workflows | CI/signing canonical entrypoints работают локально, workflow YAML удалены, Actions отключены, Sparkle signer остаётся в named macOS Keychain; production deploy не входит в срез. |
