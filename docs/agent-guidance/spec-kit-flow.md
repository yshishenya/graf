...
| Active Spec Kit slice | Work belongs to an existing `specs/<feature>/` and `tasks.md` | Use the existing spec, plan, quickstart, and tasks; do not create a duplicate slice | Focused quickstart/tests locally; required GitHub `governance-fast`, `macos-pr`, `pr-metadata` on exact PR SHA; one GitHub `release-full` on the frozen release candidate |
| Significant feature / architecture | New feature, architecture, cross-module contract, or user-visible workflow | Full Spec Kit sequence below | Focused quickstart/tests locally; required GitHub `governance-fast`, `macos-pr`, `pr-metadata` on exact PR SHA; one GitHub `release-full` on the frozen release candidate |
| High-risk product area | Capture, auth, privacy, storage, AI, deletion, diagnostics, deployment, high-risk UX, or reference-fidelity work | Full Spec Kit with mandatory clarify/checklist/analyze | Domain gates and focused quickstart/tests; required GitHub `governance-fast`, `macos-pr`, `pr-metadata` on exact PR SHA; one GitHub `release-full` on the frozen release candidate; deploy gate only for release |
...
Local `infra/scripts/ci-local.sh --fast` / `--full` are diagnostic/fallback
commands, not additional routine pre-PR gates. They do not replace required
GitHub checks or release evidence; see `release-and-validation.md`.
...
### Автоматическое независимое ревью чеклиста

Перед реализацией любого активного среза, в котором есть reviewer-owned
чеклист, агент MUST автоматически запустить отдельного независимого
read-only reviewer-агента и не ждать отдельного подтверждения пользователя для
начала этой проверки. Это не отменяет reviewer gate и не даёт основному агенту
права отмечать чеклист самостоятельно.

Reviewer-агент обязан:

- прочитать чеклист, `spec.md`, `plan.md`, `tasks.md`, конституцию и требуемые
  guidance-файлы;
- проверить каждый пункт по фактическим артефактам и проверкам, а не по
  предположению или одному сообщению агента;
- отметить `[x]` только те пункты, для которых найдено достаточное evidence;
- оставить `[ ]` для неподтверждённых пунктов и записать причину в отдельном
  отчёте ревью или в примечаниях отчёта;
- перечитать изменённый чеклист и вернуть итоговую статистику checked/unchecked
  и список использованных доказательств.

Reviewer-агент может изменять только сам reviewer-owned чеклист и отдельный
отчёт ревью. Он MUST NOT изменять код, `spec.md`, `plan.md`, `tasks.md`,
конституцию, PR, GitHub-состояние, commit, release или deploy. Он MUST NOT
отмечать пункт только потому, что реализация выглядит правдоподобной, и MUST
оставлять high-risk срез заблокированным, если после проверки остаются
неподтверждённые пункты. Основной агент продолжает работу только после
полного PASS чеклиста и остальных ворот анализа, issue sync и validation.

Если reviewer-агент завершился с ошибкой или его отчёт/изменения нельзя
проверить, чеклист считается неподтверждённым и реализация остаётся на стопе.
