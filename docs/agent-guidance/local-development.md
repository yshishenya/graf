...
В том же `status` смотри поле `app`: там указан путь установленного приложения и
признак `installed`. Если приложение пропало, `status` добавляет предупреждение с
путём. Не полагайся вместо этого на признак запущенного процесса: приложение
опознаётся по пути пакета и потому продолжает отвечать `running` даже после
исчезновения пакета. Восстановление выполняет `promote` неизменного активного
manifest из его точной копии SHA; готовый проверенный пакет лежит внутри стенда.
Копировать пакет в `/Applications` вручную нельзя.
...
`build-local-app.sh`, `run-local-app.sh` и
`run-system-audio-controlled-manual-gate.sh` оставлены как отказ с понятным
