...
def pin_model_settings(attempt) -> PromptSnapshot:
    ...
    from twobrain_rec_server.outcomes.prompt_bundle import bind_snapshot_from_metadata
    metadata = (attempt.metadata_json or {}).get("prompt_bundle")
    if metadata is not None:
        snapshot = bind_snapshot_from_metadata(snapshot, metadata)
