...
def test_empty_summary_slot_keeps_audio_and_reports_current_attempt(client, attempt_status) -> None:
    ...
    async def run() -> None:
        async with client.app_state["sessionmaker"]() as db:
            ...
            expected_event_id = str(attempt.id) if attempt_status == "failed" else None
        ...
        assert response.json()["review"]["summary_event_id"] == expected_event_id
        assert bool(response.json()["review"]["summary_updated_at"]) == (attempt_status == "failed")
    ...
    async def run():
        async with client.app_state["sessionmaker"]() as db:
            ...
            from twobrain_rec_server.outcomes.progress import (
                summary_progress,
                summary_progress_snapshot,
            )

            result = await db.get(ProcessingResult, candidate.processing_result_id)
            assert await summary_progress_snapshot(db, meeting=meeting, result=result) == (
                "partial", candidate.id, candidate.accepted_at,
            )
