...
def test_processing_dispatcher_is_ready_without_api_mediascribe_secret(client) -> None:
    client.app.state.settings.processing_enabled = True
    client.app.state.settings.temporal_address = "rec-temporal:7233"
    client.app.state.settings.mediascribe_base_url = "https://mediascribe.2brain.pro"
    client.app.state.settings.mediascribe_api_key_file = None

    ready = client.get("/api/v1/health/ready/internal", headers={"X-Internal-Health-Check": "true"})

    assert ready.status_code == 200
    assert ready.json()["checks"]["processing"] == "enabled"
    assert ready.json()["checks"]["temporal"] == "configured"
    assert ready.json()["checks"]["mediascribe"] == "dispatcher_only"


def test_production_processing_enabled_requires_temporal_and_accepts_worker_mediascribe_secret(tmp_path) -> None:
...
def test_production_processing_dispatcher_does_not_require_api_mediascribe_secret() -> None:
    settings = Settings(
        env="production",
        database_url="postgresql+asyncpg://twobrain_rec:prod-pass@rec-postgres:5432/twobrain_rec",
        minio_endpoint="rec-minio:9000",
        ...
        temporal_address="rec-temporal:7233",
        mediascribe_base_url="https://mediascribe.2brain.pro",
        mediascribe_api_key_file=None,
    )

    assert settings.processing_enabled is True
    assert settings.mediascribe_api_key_file is None
