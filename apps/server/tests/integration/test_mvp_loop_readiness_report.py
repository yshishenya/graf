...
def test_050_report_keeps_notes_action_closed_and_rollout_boundary_visible() -> None:
    report = build_default_readiness_report(
    ...
    assert "notes-action-output" not in gap_ids
    assert "production-user-rollout-evidence" in gap_ids
    assert "feature-050-closeout-report" in markdown
    assert "Recommended next action: keep 050 capped at `pilot_blocked`" in markdown
