    ...
    try:
        async with engine.connect() as connection:
            ...
            entry_count = int(
                (
                    await connection.execute(
                        text("select count(*) from meeting_target_registry_entries")
                    )
                ).scalar_one()
            )
    finally:
            ...
            await connection.execute(
                text(
                    ...
                    insert into meeting_target_registry_versions (
                        ...
                        :id, :registry_version, :schema_version, :status, :source,
                        :published_at, cast(:document_json as jsonb), :etag,
...
def test_meeting_detection_publish_migration_uses_explicit_registry_data() -> None:
    migration = _load_migration_module(
        PUBLISH_MIGRATION, "meeting_detection_publish_registry_migration"
    )
