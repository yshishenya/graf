...
def test_rename_persists_and_preserves_identity_and_escaped_rendering(client, desktop):
    ...
    before = _info(client, ready_id)
    title = "План <b>работ</b> & «команда» 😀"
    result = _rename(
        client, ready_id, f"  {title}  ", before["title_version"], desktop=desktop
    )
    assert result.status_code == 200, result.text
    assert result.json()["title"] == title
    after = _info(client, ready_id)
    assert after["title"] == title
    assert after["title_version"] != before["title_version"]
    assert after["started_at"] == before["started_at"]
    assert after["status"] == before["status"]
    html = client.get(
        f"{'/desktop' if desktop else ''}/meetings/{ready_id}", headers=auth_headers()
    ).text
    ...
    assert (
        next(x for x in listed["items"] if x["meeting_id"] == str(ready_id))["title"] == title
    )
...
def test_invalid_name_is_rejected_without_echo(client, title, code):
    ready_id = create_ready_meeting(client)
    before = _info(client, ready_id)
    result = _rename(client, ready_id, title, before["title_version"])
    assert result.status_code == 422
    assert result.json()["code"] == code
    assert title not in result.text
    assert _info(client, ready_id)["title"] == before["title"]


def test_unicode_limit_conflict_and_lost_response_retry(client):
    ready_id = create_ready_meeting(client)
    version = _info(client, ready_id)["title_version"]
    title = "😀" * 500
    first = _rename(client, ready_id, title, version)
    assert first.status_code == 200
    repeat = _rename(client, ready_id, title, version)
    assert repeat.json() == first.json()
    conflict = _rename(client, ready_id, "Второе имя", version)
    assert conflict.status_code == 409
    assert conflict.json()["title"] == title
    assert conflict.json()["title_version"] == first.json()["title_version"]
    assert (
        _rename(client, ready_id, "Второе имя", conflict.json()["title_version"]).status_code
        == 200
    )
...
def test_shared_viewers_and_other_workspace_cannot_rename(client, role):
    ...
    for headers in (
        ...
        result = _rename(
            client, ready_id, "Чужое имя", before["title_version"], headers=headers
        )
        assert result.status_code in {403, 404}
        assert before["title"] not in result.text
    html = client.get(f"/meetings/{ready_id}", headers=auth_headers_for()).text
...
def test_html_fallback_success_and_validation_preserve_draft(client):
    ready_id = create_ready_meeting(client)
    version = _info(client, ready_id)["title_version"]
    bad = _rename(client, ready_id, "x" * 501, version, accept="text/html")
    assert bad.status_code == 422
    assert "x" * 501 in bad.text and "не больше 500" in bad.text
    good = _rename(client, ready_id, "Новое имя", version, accept="text/html")
    assert good.status_code == 303
    assert good.headers["location"] == f"/meetings/{ready_id}"
...
def test_stale_ingest_snapshot_cannot_restore_old_title(client):
    ...
    async def snapshot():
        async with client.app_state["sessionmaker"]() as db:
            return await load_meeting_record(db, meeting_id=ready_id)

    stale = client.portal.call(snapshot)
    before = _info(client, ready_id)
    saved = _rename(client, ready_id, "Ручное название", before["title_version"]).json()
    ...
    async def race():
        async with client.app_state["sessionmaker"]() as a, client.app_state["sessionmaker"]() as b:
            locked = await lock_meeting_fence(
                a, workspace_id=WORKSPACE_ID, meeting_id=ready_id
            )
            if first == "calendar":
            ...
            async def second_writer():
                ...
                m = await lock_meeting_fence(b, workspace_id=WORKSPACE_ID, meeting_id=ready_id)
...
def test_cookie_rename_requires_session_bound_csrf(client, desktop):
    ...
    path = f"{'/desktop' if desktop else ''}/meetings/{ready_id}/title"
...
def test_title_version_prevents_aba_and_deletion_hides_html_draft(client):
    ready_id = create_ready_meeting(client)
    original = _info(client, ready_id)
    first = _rename(client, ready_id, "Временное имя", original["title_version"]).json()
    restored = _rename(client, ready_id, original["title"], first["title_version"]).json()
    assert restored["title_version"] != original["title_version"]
    assert (
        _rename(client, ready_id, "Устаревшая правка", original["title_version"]).status_code
        == 409
    )

    async def deleting():
        async with client.app_state["sessionmaker"]() as db:
            m = await db.get(Meeting, ready_id)
            m.deletion_state = "requested"
            await db.commit()

    client.portal.call(deleting)
    result = _rename(
        client, ready_id, "Недоступный черновик", restored["title_version"], accept="text/html"
    )
...
def test_export_preserves_full_user_title(client):
    ...
    assert (
        _rename(
            client, ready_id, title, _info(client, ready_id)["title_version"]
        ).status_code
        == 200
    )
