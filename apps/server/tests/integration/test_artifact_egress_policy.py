...
def test_allowed_transcript_download_is_server_mediated_and_audited(client) -> None:
    ...
    event_types = [event.event_type for event in audit_events(client, ready_id)]
...
def test_blocked_artifact_download_fails_closed_and_records_denied_audit(client) -> None:
    ...
    response = client.get(
        f"/api/v1/cabinet/meetings/{ready_id}/downloads/audio",
        headers=auth_headers(),
    )
...
def test_owner_default_audio_download_returns_stored_m4a_without_policy(client) -> None:
    ...
    m4a_body = add_retained_playback_m4a(
        client, ready_id, b"\x00\x00\x00\x18ftypM4A owner default"
    )
...
def test_owner_default_audio_download_returns_stored_m4a_for_workspace_default_disabled(
    ...
    m4a_body = add_retained_playback_m4a(
        client, ready_id, b"\x00\x00\x00\x18ftypM4A workspace default"
    )
...
def test_explicit_audio_download_deny_remains_blocked_for_owner(client) -> None:
    ...
    add_retained_playback_m4a(client, ready_id, b"\x00\x00\x00\x18ftypM4A explicit deny")
...
def test_owner_default_audio_download_remains_owner_only_for_permitted_non_owner(client) -> None:
    ...
    add_retained_playback_m4a(client, ready_id, b"\x00\x00\x00\x18ftypM4A non owner")
...
def test_allowed_audio_download_returns_stored_m4a_review_artifact(client) -> None:
    ...
    m4a_body = add_retained_playback_m4a(client, ready_id, b"\x00\x00\x00\x18ftypM4A download")
...
def test_audio_download_ignores_stale_processing_result_when_playback_is_available(
    ...
    replace_retained_audio_with_test_wav(client, ready_id)
    m4a_body = add_retained_playback_m4a(
        client, ready_id, b"\x00\x00\x00\x18ftypM4A stale result"
    )
...
def test_allowed_audio_download_reports_storage_unavailable_when_stored_m4a_object_is_missing(client) -> None:
    ...
    assert [(event.event_type, event.outcome, event.policy_reason) for event in audit_events(client, ready_id)] == [
        ("download_requested", "allowed", "policy_allowed"),
...
def test_package_export_policy_blocks_direct_export_even_when_transcript_is_allowed(client) -> None:
    ...
    assert [(event.event_type, event.outcome, event.artifact_class) for event in audit_events(client, ready_id)] == [
...
def test_ready_export_package_download_returns_manifest_without_storage_urls(client) -> None:
    ready_id = create_ready_meeting(client)
    set_artifact_policy(client, ready_id, transcript_download="allowed", package_export="allowed")
    created = client.post(
        f"/api/v1/cabinet/meetings/{ready_id}/exports",
        headers=auth_headers(),
        json={"artifact_classes": ["transcript"]},
    ...
    response = client.get(
        f"/api/v1/cabinet/meetings/{ready_id}/exports/{export_id}/download",
        headers=auth_headers(),
    )
...
def test_activity_endpoint_accepts_stream_prepared_audio_events(client) -> None:
    ready_id = create_ready_meeting(client)
    add_retained_playback_m4a(client, ready_id, b"\x00\x00\x00\x18ftypM4A activity")
    set_artifact_policy(client, ready_id, audio_download="allowed")
