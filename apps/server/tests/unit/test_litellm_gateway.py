...
def _snapshot():
    kind, prompt, config = desired_prompts()["graf/meeting-outcome/auto"]
    return validate_prompt_snapshot(
        name="graf/meeting-outcome/auto",
        version=3,
        prompt_type=kind,
        prompt=prompt,
        config={**config, "model": "synthetic-route", "temperature": 0},
    )
            ...
            "choices": [
                {
                    "finish_reason": "stop",
                    "message": {
                        "content": '{"synthetic_transport":true}',
                    }
                }
            ],
        }
...
@pytest.mark.asyncio
async def test_gateway_timeout_is_ambiguous_even_with_a_proxy_response(monkeypatch) -> None:
    import httpx

    class ProxyTimeoutClient(_AsyncClient):
        async def post(self, *args, **kwargs):
            self.requests.append(kwargs["json"])
            return httpx.Response(504, text="<html><h1>504 Gateway Time-out</h1><p>nginx</p></html>")

    monkeypatch.setattr(httpx, "AsyncClient", ProxyTimeoutClient)
    _AsyncClient.requests.clear()
    with pytest.raises(LiteLLMError) as raised:
        await LiteLLMGateway(
            base_url="https://litellm.example/v1", api_key="secret", timeout_seconds=600,
        ).generate(snapshot=_snapshot(), messages=[{"role": "user", "content": "synthetic"}])
    assert raised.value.code == "litellm_outcome_ambiguous"
    assert raised.value.retryable is False
    assert raised.value.egress_state == "unknown"
    assert raised.value.raw_response["http_status"] == 504
    assert "nginx" in raised.value.raw_response["body_text"]
    assert len(_AsyncClient.requests) == 1
