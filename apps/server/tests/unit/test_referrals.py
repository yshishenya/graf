...
def test_referral_risk_is_a_review_signal_and_token_shape_is_strict() -> None:
    ...
    assert (
        classify_referral_risk(ReferralRiskSignals(same_device=True, same_payment_profile=True))
        == "review"
    )
    with pytest.raises(ValueError):
...
def test_referral_links_use_configured_public_origin_not_request_host() -> None:
    source = (
        Path(__file__).parents[2] / "src/twobrain_rec_server/cabinet/web_routes/referrals.py"
...
def test_referral_routes_keep_contract_alias_and_gate_unissued_link() -> None:
    ...
    template_source = (
        ...
        / "src/twobrain_rec_server/cabinet/templates/cabinet/pages/referrals_content.html"
    ).read_text(encoding="utf-8")
