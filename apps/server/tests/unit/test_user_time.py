...
def test_billing_account_sharing_and_audit_use_viewer_time():
    ...
    try:
        ...
        profile = AccountProfileView(display_name="Synthetic", timezone="Europe/Moscow")
        page = render_settings_page(category="account", profile=profile, account_surface=
            AccountSettingsSurface(profile=profile, devices=(AccountDeviceView(
                device_id=UUID(int=1), platform_label="macOS", version_label="dev",
                status_label="Активно", last_seen_at=instant, current=True, can_revoke=False,
            ),)))
