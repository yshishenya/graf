...
(async()=>{
 ...
 let template={template_id:'id1',template_key:'personal',version:1,name:'Исходный',purpose:'Итоги',sections:['summary'],output_language:'ru',detail_level:'standard'},key='auto',patches=0,creates=0,fail=false;
 try{
  ...
  const name=page.getByLabel('Название');await name.fill('Новое');await name.press('Tab');await page.waitForFunction(()=>!GRAFSettings.pending());
