...
def build_default_launch_gaps(feature: str = "034-mvp-loop-readiness") -> list[LaunchGap]:
    ...
    return sort_launch_gaps(
        [
            ...
            *production_gaps,
            LaunchGap(
                id="single-source-finalization-evidence",
                severity="P2",
                affected_journey="local-artifact-finalization",
                current_evidence="The referenced finalization evidence predates single-source recording packages.",
                missing_evidence="Current installed-app single-source finalization evidence.",
                recommended_next_action="Record current metadata-only finalization evidence before marking this stage ready.",
                owner_area="desktop",
            ),
            LaunchGap(
...
def build_default_stages(feature: str = "034-mvp-loop-readiness") -> list[MvpLoopStage]:
    ...
    return [
        ...
        MvpLoopStage(
            ...
            launch_gap_ids=["single-source-finalization-evidence"],
            claim_impact=["desktop_loop_verified", "mvp_loop_ready"],
