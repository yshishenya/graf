...
EMPTY = {
    "Принятые решения": "Принятые решения в расшифровке не зафиксированы.",
...
def protocol_source_url(meeting_url, ref):
    parts = urlsplit(meeting_url)
    query = dict(parse_qsl(parts.query))
    query["source_result_id"] = ref["processing_result_id"]
    return urlunsplit(parts._replace(query=urlencode(query),
                                    fragment="graf-source=" + ref["transcript_segment_id"]))
...
def protocol_lines(document, *, markdown, include_evidence, meeting_url):
    ...
    def refs(row):
        ...
        for ref in row.get("source_refs", []):
            ...
            url = protocol_source_url(meeting_url, ref)
