...
def render_email_code_page(
    *,
    ...
    message_heading = {
        ...
        "email_code_wrong": "Код введен неверно",
        "email_code_expired": "Код истек",
    ...
    subtitle = (
        ...
        if not can_verify
            ...
            f"Проверьте {email}: мы отправили 6-значный код. Если аккаунта GRAF еще нет, "
...
def _login_error_message(error: str | None, *, link_flow: bool = False) -> str | None:
    ...
    if error == "ambiguous_email_recovery_required" and link_flow:
        return (
            ...
            "подтвердите другой уже подключенный способ входа — например, Яндекс ID или VK."
        )
    messages = {
        "missing_auth_context": "Нужен вход, чтобы открыть кабинет встреч.",
        "auth_handoff_invalid": "Не удалось безопасно открыть тарифы. Войдите еще раз.",
        ...
        "email_connected_relogin_required": "Email подключен к текущему профилю. Войдите снова любым сохраненным способом.",
        "yandex_connected_relogin_required": "Яндекс ID подключен к текущему профилю. Войдите снова любым сохраненным способом.",
        "vk_connected_relogin_required": "VK ID подключен к текущему профилю. Войдите снова любым сохраненным способом.",
        "sign_in_method_connected_relogin_required": "Способ входа подключен к текущему профилю. Войдите снова любым сохраненным способом.",
        ...
        "ambiguous_email_recovery_required": "Этот email связан с несколькими профилями. Вход по коду заблокирован, чтобы не открыть чужие встречи. Войдите ниже через уже подключенный Яндекс ID или VK: GRAF откроет настройки, где можно безопасно подключить email.",
        ...
        "email_code_wrong": "Проверьте цифры и попробуйте еще раз. После трех неверных попыток код блокируется.",
        "workspace_enrollment_required": "Регистрация в этом кабинете закрыта. Попросите администратора выслать приглашение.",
        "email_code_invalid": "Код не подошел и больше не действует. Запросите новый код.",
        ...
        "share_recipient_mismatch": "Этот аккаунт не совпадает с приглашенным адресом. Войдите с другим аккаунтом.",
    }
