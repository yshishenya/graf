    ...
    if credential_issue:
        ...
        rows = "".join(
            ...
            <article class="calendar-home-upcoming__row">
              {f'<time datetime="{escape(item.starts_at.isoformat())}" title="{escape(format_user_datetime(item.starts_at, show_zone=True))}" aria-label="{escape(format_user_datetime(item.starts_at, show_zone=True))}">{escape(_home_upcoming_time_label(item.starts_at, display_timezone))}</time>' if calendar_surface.preferences.show_upcoming_time else '<span class="calendar-home-upcoming__time-hidden">Время скрыто настройкой</span>'}
              <div>
