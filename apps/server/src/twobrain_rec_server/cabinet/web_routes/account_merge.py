...
def _error_copy(code: str, *, provider_label: str = "способ входа") -> str:
    ...
    return {
        ...
        "reauth_required": "Для этого действия войдите через подтвержденную веб-сессию и повторите попытку. Данные не изменены.",
        ...
        "calendar_ownership_conflict": "Ко второму профилю подключен календарь.",
        "deletion_state_conflict": "Сейчас идет закрытие профиля или удаление встречи.",
        ...
        "merge_blocked": f"{sentence_label} не подключен. Данные не изменены.",
    }.get(code, f"{sentence_label} не подключен. Данные не изменены.")
