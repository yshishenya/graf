"""Scope authenticated referral reads to the selected workspace."""

from collections.abc import Sequence

from alembic import op

revision: str = "0057_referral_workspace_scope"
down_revision: str | None = "0056_billing_catalog_write_rls"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def _policy_sql(*, scoped: bool) -> str:
    auth_public = (
        "rec_context_kind() = 'auth_public' and workspace_id = rec_current_workspace_id() and "
        if scoped
        else "rec_context_kind() = 'auth_public' and "
    )
    return (
        ...
        f"or ({auth_public}(invitee_user_id = rec_current_user_id() or inviter_user_id = rec_current_user_id())) "
...
def upgrade() -> None:
    ...
    op.execute(_policy_sql(scoped=True))
...
def downgrade() -> None:
    if op.get_bind().dialect.name != "postgresql":
        return
    op.execute("drop policy if exists referral_attributions_tenant_isolation on referral_attributions")
    op.execute(_policy_sql(scoped=False))
