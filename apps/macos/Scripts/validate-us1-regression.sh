...
RUN_COREAUDIO_PROBES="${GRAF_RUN_COREAUDIO_PROBES:-${TWO_BRAIN_RUN_COREAUDIO_PROBES:-0}}"
if [ "${GRAF_REQUIRE_RUNTIME_PROOF:-${TWO_BRAIN_REQUIRE_RUNTIME_PROOF:-0}}" = "1" ]; then
...
run_coreaudio_check() {
  ...
  TIMEOUT_SECONDS="$2"
  shift 2
  OUTPUT_FILE=$(mktemp "${TMPDIR:-/tmp}/graf-check.XXXXXX")
  TIMEOUT_FILE=$(mktemp "${TMPDIR:-/tmp}/graf-check-timeout.XXXXXX")
  rm -f "$TIMEOUT_FILE"
...
if [ "$RUN_COREAUDIO_PROBES" = "1" ]; then
  PROBE_TIMEOUT_SECONDS="${GRAF_RUNTIME_PROBE_TIMEOUT_SECONDS:-${TWO_BRAIN_RUNTIME_PROBE_TIMEOUT_SECONDS:-20}}"
  ...
  if ! echo "$RUNTIME_PROBE_OUTPUT" | rg -q "Runtime Core Audio publication proof: (ACCEPTED|BLOCKED)"; then
    ...
    if [ "${GRAF_REQUIRE_RUNTIME_PROOF:-${TWO_BRAIN_REQUIRE_RUNTIME_PROOF:-0}}" = "1" ]; then
      echo "Set GRAF_REQUIRE_RUNTIME_PROOF=1 only when runtime probe can be executed in this environment." >&2
      exit 1
    fi
  fi

  ROUTE_TIMEOUT_SECONDS="${GRAF_ROUTE_PROBE_TIMEOUT_SECONDS:-${TWO_BRAIN_ROUTE_PROBE_TIMEOUT_SECONDS:-20}}"
  ...
  echo "Set GRAF_RUN_COREAUDIO_PROBES=1 to run them with timeouts."
  echo "Set GRAF_REQUIRE_RUNTIME_PROOF=1 for pre-release proof enforcement."
fi
