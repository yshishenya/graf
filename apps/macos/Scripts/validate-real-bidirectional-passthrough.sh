...
if ! swift test --package-path "$MACOS_DIR" --disable-swift-testing --filter 'AppIOHealthTests|LivePassthroughPolicyTests|SharedAudioMemoryCompatibilityTests|LatencyGateTests|RouteVerificationTests|BrowserTargetEvidenceTests|RouteInvalidationTests|DiagnosticRedactionTests'; then
  ...
  if [ -n "$TEST_BUNDLE_BINARY" ]; then
    ...
    swift test --package-path "$MACOS_DIR" --disable-swift-testing --skip-build --filter 'AppIOHealthTests|LivePassthroughPolicyTests|SharedAudioMemoryCompatibilityTests|LatencyGateTests|RouteVerificationTests|BrowserTargetEvidenceTests|RouteInvalidationTests|DiagnosticRedactionTests'
  else
    exit 1
  fi
fi
