...
if [ -x "$SCRIPT_DIR/validate-low-resource-audio.sh" ]; then
  sh "$SCRIPT_DIR/validate-low-resource-audio.sh"
else
  echo "low-resource audio validation: blocked (script missing)"
  exit 1
fi
