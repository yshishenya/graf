...
final class DesktopUploadQueueTests: XCTestCase {
    ...
    func testV5ProfileRequiresExactFinalFilenames() {
        let valid = makeV5ArtifactProfile()
        var renamed = valid
        renamed.trackCompleteness[1].fileName = "unexpected.wav"

        XCTAssertTrue(valid.isV5Package)
        XCTAssertFalse(renamed.isV5Package)
    }

    func testV5PackageQueuesOnlyCanonicalWAVAndRequiredPlaybackM4A() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let package = try makeV5RecordingPackage(
            root: root,
            directoryId: "v5-package",
        ...
        XCTAssertTrue(item.artifactProfile.isV5Package)
        XCTAssertTrue(item.artifactProfile.isUploadable)
        XCTAssertEqual(item.artifactProfile.trackCompleteness.map(\.transportRole), [.manifest, .media, .playback])
        XCTAssertEqual(
            item.artifactProfile.trackCompleteness.map(\.fileName),
            ["manifest.json", "meeting-transcription.wav", "meeting-review.m4a"]
        )
        XCTAssertEqual(
            URL(fileURLWithPath: item.transcriptionAudioPath).resolvingSymlinksInPath(),
            package.transcriptionURL.resolvingSymlinksInPath()
        )
    }

    func testProcessDueItemsPublishesMonotonicIntermediateV5Progress() async throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        _ = try makeV5RecordingPackage(
            root: root,
            directoryId: "v5-progress-package",
            sessionId: "v5-progress-session"
        )
        let collector = UploadProgressCollector()
        let service = DesktopUploadQueueService(
            queueURL: root.appendingPathComponent("queue.json"),
            recordingsRootURL: root,
            client: ProgressPublishingUploadClient(),
            clock: { Date(timeIntervalSince1970: 100) }
        )
        ...
        XCTAssertEqual(try XCTUnwrap(finished.first).state, .uploaded)
        XCTAssertGreaterThanOrEqual(observed.count, 4)
        XCTAssertEqual(try XCTUnwrap(observed.first), 0, accuracy: 0.0001)
        XCTAssertTrue(zip(observed, observed.dropFirst()).allSatisfy { $0 <= $1 })
        XCTAssertTrue(observed.contains { $0 > 0 && $0 < 1 })
        XCTAssertNotEqual(observed[1], 0.5, accuracy: 0.0001)
    }
    ...
    func testUploadQueueSummaryExplainsBlockedLocalRecordingReasons() {
        let historicalSummary = DesktopUploadQueueSummary(
            primaryItem: makeQueueItem(
                state: .blocked,
                retryMode: .manualOnly,
    ...
    func testLocalPurgeRefusesPathsOutsideRecordingRootAndReportsFailedAck() async throws {
        ...
        let outsideCanonicalAudio = outsideDirectory.appendingPathComponent("meeting-transcription.wav")
        let outsideReviewAudio = outsideDirectory.appendingPathComponent("meeting-review.m4a")
        try Data(repeating: 3, count: 16).write(to: outsideManifest)
        try Data(repeating: 4, count: 16).write(to: outsideCanonicalAudio)
        try Data(repeating: 5, count: 16).write(to: outsideReviewAudio)
        ...
        item.microphonePath = outsideCanonicalAudio.path
        item.systemAudioPath = outsideReviewAudio.path
        try JSONEncoder.uploadQueueTestEncoder
            .encode(DesktopUploadQueueDocument(updatedAt: item.updatedAt, items: [item]))
            .write(to: queueURL, options: [.atomic])
        let task = try makeLocalPurgeTask(meetingId: meetingId)
    ...
    func testScanPersistsRecordingMetadataFromManifestStartStopAndScopeTitle() throws {
        ...
        XCTAssertEqual(item.recordingMetadata?.recordingStoppedAt, Date(timeIntervalSince1970: 11))
    ...
    func testRefreshPreservesPersistedRecordingMetadataAndStableIdentityWhenScopeChanges() throws {
        ...
        let changedManifest = try makeManifest(
            directoryId: "package-1",
            sessionId: "session-1",
            transcriptionURL: package.transcriptionURL,
            reviewURL: package.reviewURL,
    ...
    func testMissingFilesConsentAndPermissionsStillBlockUpload() throws {
        ...
        _ = try makeRecordingPackage(
            root: root,
            directoryId: "missing-canonical-package",
            sessionId: "missing-canonical-session",
            includeCanonicalAudio: false
        )
        _ = try makeRecordingPackage(
            root: root,
        ...
        XCTAssertEqual(byDirectory["missing-canonical-package"]?.state, .blocked)
        XCTAssertEqual(byDirectory["missing-canonical-package"]?.failureReason, "local_artifacts_not_uploadable")
    ...
    func testRefreshOfV5PackagePreservesExistingUploadSession() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        _ = try makeRecordingPackage(root: root, directoryId: "reenqueue-package-m4a", sessionId: "reenqueue-session-m4a")
        let queueURL = root.appendingPathComponent("queue.json")
        let service = DesktopUploadQueueService(
            queueURL: queueURL,
            recordingsRootURL: root,
        ...
        let linked = item.withTransition(
            ...
            serverTruth: ServerTruthFingerprint(
                ...
                uploadSessionId: "server-upload-session-v5",
                acceptedBytesByTrack: ["manifest": 32],
                expectedTrackRoles: ["media", "playback", "manifest"],
                desktopTruthRule: "server_ranges_authoritative"
            )
        )
        let document = DesktopUploadQueueDocument(
        ...
        XCTAssertEqual(refreshed.uploadSessionId, "server-upload-session-v5")
        XCTAssertEqual(refreshed.serverTruth.uploadSessionId, "server-upload-session-v5")
        XCTAssertEqual(refreshed.serverTruth.acceptedBytesByTrack["manifest"], 32)
        XCTAssertEqual(
            refreshed.serverTruth.expectedTrackRoles,
            ["media", "playback", "manifest"]
        )
        XCTAssertEqual(
    ...
    func testProcessDueItemsReconcilesBeforeUploadAndPersistsServerRanges() async throws {
        ...
        let client = ReconcileThenUploadClient(
            ...
            result: DesktopUploadResult(
                ...
                serverTruth: ServerTruthFingerprint(
                    ...
                    mediaRevisionId: "server-media-revision-us3",
                    uploadSessionId: "server-upload-session-us3",
                    acceptedBytesByTrack: ["media": 128],
                    desktopTruthRule: "server_ranges_authoritative"
                )
            )
        )
        ...
        XCTAssertEqual(uploadedItem.serverTruth.acceptedBytesByTrack["media"], 64)
        XCTAssertEqual(savedItem.state, .uploaded)
        XCTAssertEqual(savedItem.serverTruth.acceptedBytesByTrack["media"], 128)
        ...
        let client = ReconcileThenUploadClient(
            reconciliation: DesktopUploadReconciliation(
                serverTruth: ServerTruthFingerprint(
                    ...
                    acceptedBytesByTrack: ["media": 128, "playback": 128, "manifest": 64],
                    desktopTruthRule: "server_ranges_authoritative"
                )
            ),
            result: DesktopUploadResult(
    ...
    func testReconciliationConflictBlocksUploadAndPreservesServerTruth() async throws {
        ...
        XCTAssertEqual(savedItem.serverTruth.acceptedBytesByTrack["media"], 64)
        XCTAssertEqual(savedItem.retentionDecision.decision, .manualOnly)
        XCTAssertTrue(savedItem.retentionDecision.localArtifactsRetained)
    }

    func testConflictSummaryCopyIsSafeAndDoesNotLeakLocalPaths() {
        let localPath = "/Users/test/private/recordings/package/meeting-transcription.wav"
    ...
    func testCustodySafeReportRedactionKeepsIncidentMetadataOnly() throws {
        let item = makeQueueItem(
            state: .blocked,
            retryMode: .manualOnly,
            failureReason: "/Users/test/private/recordings/package/meeting-transcription.wav Bearer leaked-token",
            syncConflictState: .serverMeetingDeleted,
    ...
    func testRetentionExpiryMarksRecoverableConflictWithoutDeletingArtifacts() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: expired.transcriptionAudioPath))
        XCTAssertTrue(FileManager.default.fileExists(atPath: expired.reviewAudioPath))
    }
    ...
    func testMicrophoneSampleGraphMetadataDoesNotChangeUploadCompletenessDecision() throws {
        ...
        XCTAssertTrue(item.artifactProfile.isUploadable)
        XCTAssertEqual(item.artifactProfile.trackCompleteness.map(\.transportRole), [.manifest, .media, .playback])
        XCTAssertGreaterThan(item.artifactProfile.trackCompleteness[1].byteCount, 0)
        XCTAssertGreaterThan(item.artifactProfile.trackCompleteness[2].byteCount, 0)
    ...
    func testV5PackageIncludesRequiredReviewPlaybackDescriptor() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        _ = try makeRecordingPackage(
            root: root,
    ...
    func testInvalidReviewM4ABlocksV5Package() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let package = try makeRecordingPackage(
            root: root,
    ...
    func testWAVAtReviewPathBlocksV5Package() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let package = try makeRecordingPackage(
            root: root,
    ...
    func testIncompletePackageBecomesBlockedAndManualOnly() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        _ = try makeRecordingPackage(root: root, directoryId: "package-2", sessionId: "session-2", includeCanonicalAudio: false)
        let service = DesktopUploadQueueService(
            queueURL: root.appendingPathComponent("queue.json"),
            recordingsRootURL: root,
            client: nil,
        ...
        XCTAssertEqual(items.first?.failureCategory, .localResource)
    }

    func testV5CaptureFailureUsesExactLocalReasonAndSavedAudioDuration() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let package = try makeV5RecordingPackage(
            root: root,
            directoryId: "capture-failed-package",
            sessionId: "capture-failed-session"
        )
        ...
        XCTAssertEqual(item.failureReason, "aec_capture_failed")
        XCTAssertLessThan(item.artifactProfile.durationSeconds, 40)
        XCTAssertEqual(
        ...
        staleItem.failureReason = "local_recording_package_not_uploadable"
        try JSONEncoder.uploadQueueTestEncoder
            .encode(DesktopUploadQueueDocument(updatedAt: staleItem.updatedAt, items: [staleItem]))
            .write(to: root.appendingPathComponent("queue.json"), options: [.atomic])
        let refreshedService = DesktopUploadQueueService(
            queueURL: root.appendingPathComponent("queue.json"),
            recordingsRootURL: root,
            client: nil,
    ...
    func testLocalPlaybackRejectsMissingTrackAndPathOutsideRecordingRoot() throws {
        ...
        try Data([0]).write(to: outsideRoot.appendingPathComponent("meeting-review.m4a"))
        let queueURL = root.appendingPathComponent("queue.json")
        var item = makeQueueItem(id: "unsafe-playback", state: .blocked, retryMode: .manualOnly)
        item.directoryPath = outsideRoot.path
        try JSONEncoder.uploadQueueTestEncoder
            .encode(DesktopUploadQueueDocument(updatedAt: item.updatedAt, items: [item]))
            .write(to: queueURL, options: [.atomic])
        let service = DesktopUploadQueueService(
        ...
        item.directoryPath = root.appendingPathComponent("missing-track", isDirectory: true).path
        item.artifactProfile.trackCompleteness.removeAll { $0.transportRole == .playback }
        try FileManager.default.createDirectory(atPath: item.directoryPath, withIntermediateDirectories: true)
        try Data([0]).write(to: URL(fileURLWithPath: item.reviewAudioPath))
        try JSONEncoder.uploadQueueTestEncoder
            .encode(DesktopUploadQueueDocument(updatedAt: item.updatedAt, items: [item]))
            .write(to: queueURL, options: [.atomic])
        let missingTrackService = DesktopUploadQueueService(
            queueURL: queueURL,
            recordingsRootURL: root,
            client: nil
        )

        XCTAssertThrowsError(try missingTrackService.localPlaybackURL(itemId: item.id))

        let unreadablePackage = try makeV5RecordingPackage(
            root: root,
            directoryId: "unreadable-playback",
            sessionId: "unreadable-playback-session"
        )
        let unreadableService = DesktopUploadQueueService(
            queueURL: root.appendingPathComponent("unreadable-queue.json"),
            recordingsRootURL: root,
            client: nil
        )
    ...
    private func makeQueueItem(
        ...
        return DesktopUploadQueueItem(
            ...
            microphonePath: "/tmp/directory/meeting-transcription.wav",
            systemAudioPath: "/tmp/directory/meeting-review.m4a",
            state: state,
    ...
    private func makeRecordingPackage(
        ...
        let package = TestRecordingPackage(
            ...
            transcriptionURL: directoryURL.appendingPathComponent("meeting-transcription.wav"),
            reviewURL: directoryURL.appendingPathComponent("meeting-review.m4a")
        )
        if includeCanonicalAudio {
            try writeTestTranscriptionAudio(to: package.transcriptionURL)
        }
        if includeReviewAudio {
            try writeTestReviewAudio(to: package.reviewURL)
        }
    ...
    private func makeV5RecordingPackage(
        root: URL,
        directoryId: String,
        sessionId: String
    ) throws -> V5TestRecordingPackage {
        let directoryURL = root.appendingPathComponent(directoryId, isDirectory: true)
        try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        let package = V5TestRecordingPackage(
            directoryURL: directoryURL,
            manifestURL: directoryURL.appendingPathComponent("manifest.json"),
            transcriptionURL: directoryURL.appendingPathComponent("meeting-transcription.wav"),
            reviewURL: directoryURL.appendingPathComponent("meeting-review.m4a")
        ...
        let stoppedAt = Date(timeIntervalSince1970: 11)
        let manifest = LocalRecordingManifest(
            ...
            tracks: [
                LocalRecordingTrack(
                    ...
                    status: .saved,
                    fileName: package.transcriptionURL.lastPathComponent,
                    format: "wav-pcm-s16le",
                    sampleRate: 16_000,
                    channelCount: 1,
                    bitsPerSample: 16,
                    durationMs: 1_000,
                    byteCount: Int64(try Data(contentsOf: package.transcriptionURL).count),
                    sha256: try testSHA256(package.transcriptionURL),
                    frameCount: 16_000,
                    timelineStartMs: 0,
                ...
                LocalRecordingTrack(
                    ...
                    status: .saved,
                    fileName: package.reviewURL.lastPathComponent,
                    format: "m4a-aac-lc",
                    sampleRate: 48_000,
                    channelCount: 1,
                    bitsPerSample: 0,
                    durationMs: 1_000,
                    byteCount: Int64(try Data(contentsOf: package.reviewURL).count),
                    sha256: try testSHA256(package.reviewURL),
                    frameCount: 48_000,
            ...
            scopeApproval: captureScope(sourceDisplayName: "Display"),
            permissions: SystemAudioPermissionSnapshot(
                microphone: .granted,
                systemAudio: .granted,
    ...
    private func makeV5ArtifactProfile() -> ArtifactCompletenessProfile {
    ...
    private func writeTestTranscriptionAudio(to url: URL) throws {
    ...
    private func testSHA256(_ url: URL) throws -> String {
    ...
    private func makeManifest(
        ...
        let transcriptionExists = FileManager.default.fileExists(atPath: transcriptionURL.path)
        let reviewExists = FileManager.default.fileExists(atPath: reviewURL.path)
        let transcriptionData = transcriptionExists ? try Data(contentsOf: transcriptionURL) : Data()
        let reviewData = reviewExists ? try Data(contentsOf: reviewURL) : Data()
        let tracks = [
            LocalRecordingTrack(
                trackId: "canonical-media",
                role: .mixedMeetingAudio,
                sourceKind: .canonicalMix,
                mediaScribeField: .mediaFile,
                status: transcriptionExists ? .saved : .missing,
                fileName: "meeting-transcription.wav",
                format: "wav-pcm-s16le",
                sampleRate: 16_000,
                channelCount: 1,
                bitsPerSample: 16,
                durationMs: 1_000,
                byteCount: Int64(transcriptionData.count),
                sha256: transcriptionExists ? try testSHA256(transcriptionURL) : nil,
                frameCount: 160_000,
                timelineStartMs: 0,
                timelineAligned: true
            ),
            LocalRecordingTrack(
                ...
                sourceKind: .canonicalMix,
                mediaScribeField: .playbackFile,
                status: reviewExists ? .saved : .missing,
                fileName: "meeting-review.m4a",
                format: "m4a-aac-lc",
                sampleRate: 48_000,
                channelCount: 1,
                bitsPerSample: 0,
                durationMs: 1_000,
                byteCount: Int64(reviewData.count),
                sha256: reviewExists ? try testSHA256(reviewURL) : nil,
                frameCount: 48_000,
                aacPresentationFrameDelta: 0,
                timelineStartMs: 0,
                timelineAligned: true
    ...
    private struct V5TestRecordingPackage {
        let directoryURL: URL
        let manifestURL: URL
        let transcriptionURL: URL
        let reviewURL: URL
    ...
    private actor UploadProgressCollector {
        private var snapshots: [[DesktopUploadQueueItem]] = []

        func append(_ items: [DesktopUploadQueueItem]) {
            snapshots.append(items)
        }

        func progressFractions(for itemId: String) -> [Double] {
            snapshots.compactMap { items in
                items.first(where: { $0.id == itemId })?.progressFraction
            }
        }
    }

    private final class ProgressPublishingUploadClient: @unchecked Sendable, DesktopUploadClientProtocol {
        func reconcile(_: DesktopUploadQueueItem) async throws -> DesktopUploadReconciliation? {
            nil
        }

        func upload(_ item: DesktopUploadQueueItem) async throws -> DesktopUploadResult {
            try await upload(item) { _ in }
        }
        ...
        func upload(
            ...
            let tracks = Dictionary(
                uniqueKeysWithValues: item.artifactProfile.trackCompleteness.map { ($0.transportRole.rawValue, $0.byteCount) }
            )
            let common = ServerTruthFingerprint(
                meetingId: "progress-meeting",
                mediaRevisionId: "progress-revision",
                uploadSessionId: "progress-session",
                serverStatus: "uploading",
                processingStatus: "not_submitted",
                expectedTrackRoles: ["manifest", "media", "playback"]
            )
            let manifestBytes = tracks["manifest", default: 0]
            let mediaBytes = tracks["media", default: 0]
            let playbackBytes = tracks["playback", default: 0]

            try await onProgress(ServerTruthFingerprint(
                meetingId: common.meetingId,
                mediaRevisionId: common.mediaRevisionId,
                uploadSessionId: common.uploadSessionId,
                serverStatus: common.serverStatus,
                processingStatus: common.processingStatus,
                acceptedBytesByTrack: ["manifest": manifestBytes, "media": mediaBytes / 4],
                expectedTrackRoles: common.expectedTrackRoles
            ))
            try await onProgress(ServerTruthFingerprint(
                meetingId: common.meetingId,
                mediaRevisionId: common.mediaRevisionId,
                ...
                serverStatus: common.serverStatus,
                processingStatus: common.processingStatus,
                acceptedBytesByTrack: ["manifest": manifestBytes, "media": mediaBytes * 3 / 4, "playback": playbackBytes / 3],
                expectedTrackRoles: common.expectedTrackRoles
            ))
            ...
            return DesktopUploadResult(
                ...
                serverTruth: ServerTruthFingerprint(
                    ...
                    serverStatus: "ingested_pending_processing",
                    processingStatus: "pending_processing",
                    acceptedBytesByTrack: tracks,
                    expectedTrackRoles: common.expectedTrackRoles,
                    finalizedAt: Date(timeIntervalSince1970: 101)
                )
            )
        }
