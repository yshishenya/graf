...
final class DevChannelPackagingTests: XCTestCase {
    ...
    func testDevInstallerIsAtomicAndDoesNotTouchProduction() throws {
        ...
        XCTAssertTrue(source.contains("swift \"$APP_LIFECYCLE\" swap"))
    ...
    func testDevLifecycleUsesNativeApplicationTermination() throws {
        ...
        XCTAssertTrue(source.contains("expectedDevDestination"))
        XCTAssertTrue(source.contains("app swap destination must be /Applications/GRAF Dev.app"))
        XCTAssertTrue(source.contains("assertDevBundle(staged"))
        XCTAssertTrue(source.contains("assertDevBundle(installed"))
