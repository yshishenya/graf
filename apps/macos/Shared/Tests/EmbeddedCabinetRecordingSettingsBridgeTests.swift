...
final class EmbeddedCabinetRecordingSettingsBridgeTests: XCTestCase {
    ...
    func testActualWebKitFormSavesAndRefreshesFromNativeNotification() async throws {
        ...
        _ = try await evaluate("const s = document.querySelector('[data-recording-target=one]'); s.focus(); s.value = 'always'; s.dispatchEvent(new Event('change', {bubbles:true}));", in: webView)
