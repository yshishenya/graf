...
final class EmbeddedCabinetRecordingSettingsBridgeTests: XCTestCase {
    ...
    func testActualWebKitFormSavesAndRefreshesFromNativeNotification() async throws {
        ...
        let autosave = try String(contentsOf: cabinet.appendingPathComponent("static/cabinet/settings-autosave.js"), encoding: .utf8)
        webView.loadHTMLString("<!doctype html><html><body><meta name='graf-time-user' content='a'><meta name='graf-workspace' content='w'>\(html)<script>\(autosave)</script><script>\(script)</script></body></html>", baseURL: origin)
        ...
        try await waitUntil("!GRAFSettings.pending()", in: webView)
